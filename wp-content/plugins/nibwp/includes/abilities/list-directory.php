...
wp_register_ability('nibwp/list-directory', [
    ...
    'meta' => [
        ...
        'mcp' => ['public' => true],
        'annotations' => [
            'instructions' => 'TIP: AI-written PHP plugins live in wp-content/nibwp-sandbox/. Check wp-content/nibwp-sandbox/.crashed to see if safe mode is active.',
            'readonly' => true,
            'destructive' => false,
            'idempotent' => true,
        ],
 ...
 */
function nibwp_list_directory(array $input = [])
{
    ...
    $resolved = nibwp_resolve_path($path, must_exist: true);
    ...
    $all_entries = $recursive
        ? nibwp_collect_recursive_entries($resolved, $pattern, $include_hidden, $max_depth)
        : nibwp_collect_flat_entries($resolved, $pattern, $include_hidden);
 ...
 */
function nibwp_collect_recursive_entries($resolved, $pattern, $include_hidden, $max_depth)
{
    $entries = [];
    ...
    foreach ($iterator as $item) {
        ...
        $entry = nibwp_build_entry($item, $pattern, $include_hidden);
        if ($entry !== null) {
            $entries[] = $entry;
        }
    }
 ...
 */
function nibwp_collect_flat_entries($resolved, $pattern, $include_hidden)
{
    ...
    while (($filename = readdir($dir_handle)) !== false) {
        ...
        $entry = nibwp_build_entry($info, $pattern, $include_hidden);
        if ($entry !== null) {
            $entries[] = $entry;
        }
    }
 ...
 */
function nibwp_build_entry($info, $pattern, $include_hidden)
{
