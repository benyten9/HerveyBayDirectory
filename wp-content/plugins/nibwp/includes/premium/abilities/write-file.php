...
wp_register_ability('nibwp/write-file', [
    'label' => __('Write File', domain: 'nibwp'),
    'description' => __(
        'Writes content to a file on the server filesystem. PHP files (*.php) can ONLY be written to the sandbox directory (wp-content/nibwp-sandbox/). Non-PHP files can go anywhere under ABSPATH. Supports both UTF-8 text and base64-encoded binary content. Automatically creates parent directories when needed.',
        domain: 'nibwp',
    ),
    'category' => 'filesystem',
    'input_schema' => [
    ...
    'execute_callback' => 'nibwp_write_file',
    ...
    'meta' => [
        ...
        'annotations' => [
            'instructions' => implode("\n", [
                'PHP FILE SANDBOX:',
                '- PHP files (*.php) can ONLY be written to: wp-content/nibwp-sandbox/',
                '- Use a path like "wp-content/nibwp-sandbox/my-feature.php"',
                ...
                '- After fixing, delete the file "wp-content/nibwp-sandbox/.crashed"',
 ...
 */
function nibwp_decode_write_content(string $content, string $encoding): string|WP_Error
{
    if ($encoding === 'base64') {
 ...
 */
function nibwp_write_file($input)
{
    ...
    $content = nibwp_decode_write_content((string) $input['content'], $encoding);
