...
wp_register_ability('nibwp/delete-file', [
    'label' => __('Delete File', domain: 'nibwp'),
    'description' => __(
    ...
    'execute_callback' => 'nibwp_delete_file',
    ...
    'meta' => [
        ...
        'annotations' => [
            'instructions' => implode("\n", [
                'SANDBOX NOTES:',
                '- Files in wp-content/nibwp-sandbox/ (the PHP sandbox) can be deleted.',
                '- To exit safe mode after a crash, delete: wp-content/nibwp-sandbox/.crashed',
            ]),
            'readonly' => false,
            'destructive' => true,
            'idempotent' => true,
 ...
 */
function nibwp_delete_file($input)
{
    $resolved = nibwp_resolve_path((string) $input['path'], must_exist: false);
    ...
    // Delete a directory.
    if (is_dir($resolved)) {
        return nibwp_delete_directory($resolved, $recursive);
    }
 ...
 * @return array|WP_Error
 */
function nibwp_delete_directory($resolved, $recursive)
{
