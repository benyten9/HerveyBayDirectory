...
wp_register_ability('nibwp/enable-file', [
    'label' => __('Enable File', domain: 'nibwp'),
    'description' => __(
        'Re-enables a previously disabled sandbox file by removing the ".disabled" suffix from its filename. Only operates on files inside the sandbox directory (wp-content/nibwp-sandbox/). Idempotent: enabling a file that is not disabled succeeds with enabled=false.',
        domain: 'nibwp',
    ),
    'category' => 'filesystem',
    'input_schema' => [
        ...
        'properties' => [
            'path' => [
                'type' => 'string',
                'description' => 'Path to the .disabled file to re-enable. Can be the original filename (without .disabled suffix) or the full disabled filename. Relative paths are resolved from ABSPATH. Must be inside wp-content/nibwp-sandbox/.',
                'minLength' => 1,
            ],
        ],
        'required' => ['path'],
    ...
    'execute_callback' => 'nibwp_enable_file',
    ...
    'meta' => [
        ...
        'annotations' => [
            'instructions' => implode("\n", [
                'SANDBOX NOTES:',
                '- Only files inside wp-content/nibwp-sandbox/ (the PHP sandbox) can be enabled.',
 ...
 */
function nibwp_enable_file($input)
{
    $path = (string) $input['path'];

    // Normalize: ensure we are looking for the .disabled version of the file.
    $disabled_path = nibwp_is_disabled_file($path) ? $path : $path . '.disabled';

    $resolved = nibwp_resolve_path($disabled_path, must_exist: true);

    // If the .disabled file was not found, check whether the original file already exists (idempotent).
    if (is_wp_error($resolved) && !nibwp_is_disabled_file($path)) {
        $original_resolved = nibwp_resolve_path($path, must_exist: true);
    ...
    $sandbox_check = nibwp_validate_sandbox_path($resolved);
    ...
    if (!nibwp_is_disabled_file($resolved)) {
        return [
