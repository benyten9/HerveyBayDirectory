...
Sourced from the official EtchWP docs (docs.etchwp.com) and Kevin Geary's patterns (patterns.etchwp.com). This reference covers the rendering engine, component system, responsive approach, and block types this skill's payloads target.
...
```json
{
  "blockName": "etch/condition",
  "attrs": {
    "condition": { "leftHand": "slots.footer.empty", "operator": "isFalsy", "rightHand": null },
    "conditionString": "!slots.footer.empty"
  },
...
| `etch/svg` | Uploaded SVG file | Only `tag`, `attributes`, `styles`. `attributes.src` = WP attachment ID (numeric); optional `attributes.stripColors`. Markup in `innerHTML` is ignored — Etch prints its default logo glyph. **Inline icons** are `etch/element` `tag: "svg"` with `path` / `circle` / `rect` … children (json-schema.md § Inline icons). |
...
|---|---|
| `var(--bg-ultra-light)` | `var(--bg-ultra-light, #f5f5f5)` — tinted by the brand color; for neutral grey use `var(--neutral-ultra-light, #f2f2f2)` |
| `var(--content-gap)` | `var(--content-gap, 1rem)` |
| `var(--content-width)` | `var(--content-width, 1366px)` |
| `var(--h1)` | `var(--h1, 2.5rem)` |
| `var(--h2)` | `var(--h2, 2.25rem)` |
...
Write grid columns out (`repeat(2, minmax(0, 1fr))`, `3fr 2fr`) — structural values need no token. Headings take a rem fallback, never `clamp()`: step down a token at a container breakpoint instead.
...
## Summary of authoring rules derived from the docs
