...
- [ ] H1 `line-height`: `--heading-line-height` (1.15).
- [ ] Subtitle `font-size`: `--text-m` (1rem) or `--text-l` (1.125rem).
- [ ] Subtitle `color`: `--text-dark-muted` on a light hero, `--text-light-muted` on a dark one.
- [ ] Background: `--white` / `--neutral-ultra-light` / `--bg-dark`; raw decorative hex inside brand-scoped files only.
