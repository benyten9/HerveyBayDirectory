...
3. Synthesize the payload with a `core/shortcode` block wrapping the chosen plugin's shortcode:
...
- [ ] Wrapper `background`: `--neutral-ultra-light` / `--white`.
...
## Responsive
- [ ] Wrapper width: `max-inline-size: min(100%, 42rem)` — forms don't need to stretch full-width.
