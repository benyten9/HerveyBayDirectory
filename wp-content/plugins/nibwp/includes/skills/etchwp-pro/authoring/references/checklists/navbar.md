...
- [ ] Border-bottom on a light navbar: `var(--border-size, 1px) solid var(--border-color-dark, rgba(0, 0, 0, 0.2))`. On a dark navbar use `--border-color-light` (white at 20%) — on a light one it is invisible.
...
- [ ] Mobile menu uses `position: fixed; inset: 68px 0 0 0;` (the navbar's measured height) to slide down full-width.
