...
Any payload that needs JS-powered interactivity MUST use the libraries listed below. Never introduce a different library for the same purpose. Consistency means a site loads each dependency once and every component works together.
...
**Init pattern** (base64-encoded in `attrs.script.code`):
...
.component__accordion-item {
  border-block-end: var(--border-size, 1px) solid var(--border-color-dark, rgba(0, 0, 0, 0.2));
}
...
Components assume the page loads these CDNs when the corresponding component type is present. The Etch builder handles loading. For standalone use, include in the page `<head>`:
