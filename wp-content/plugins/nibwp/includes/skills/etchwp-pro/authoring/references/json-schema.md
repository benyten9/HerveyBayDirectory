...
| `type` | string | Lowercase: `"element"` \| `"component"` \| `"layout"` \| `"template"`. |
...
- `etch/svg` — an **uploaded** SVG file. Its only attributes are `tag`, `attributes` and `styles`: `attrs.attributes.src` is the WP attachment ID (numeric), and the optional `attrs.attributes.stripColors` removes the file's own colors so CSS can set them. Markup in `innerHTML` is ignored and Etch prints its default logo glyph instead. For an inline icon use `etch/element` (below).

### Inline icons

Build the SVG from `etch/element` blocks: the outer one has `tag: "svg"` and the SVG attributes, and each shape (`path`, `circle`, `rect`, `line`, `polyline`, `polygon`, `g`) is a child `etch/element` carrying its own attributes (`d`, `cx`, `r`, …). Shapes need no class — SVG primitives inside an icon are exempt from `element_without_class`.
...
```json
{
    ...
    "attrs": {
        "tag": "svg",
        "attributes": { "class": "acme-feature__icon", "viewBox": "0 0 24 24", "fill": "none", "stroke": "currentColor", "stroke-width": "2", "aria-hidden": "true" },
        "styles": ["acme-feature__icon-style"]
    },
    "innerBlocks": [
        { "blockName": "etch/element", "attrs": { "tag": "circle", "attributes": { "cx": "12", "cy": "12", "r": "10" } }, "innerBlocks": [] },
        { "blockName": "etch/element", "attrs": { "tag": "path", "attributes": { "d": "M8 12l3 3 5-6" } }, "innerBlocks": [] }
    ]
}
```

An uploaded logo, by contrast, is one block: `{ "blockName": "etch/svg", "attrs": { "attributes": { "src": 512, "class": "acme-logo" }, "styles": ["acme-logo-style"] } }`.
...
```json
{
    "alpha-cta-banner__heading-style": {
        ...
        "collection": "default",
        "css": "font-size: var(--h2, 2.25rem); font-weight: 800; margin: 0; @container (inline-size < to-rem(600px)) { font-size: var(--h3, 1.5rem); }",
        "readonly": false
    }
}
```
...
```json
{
    ...
    "styles": {
        "etched-primary-button-style": {
            ...
            "collection": "default",
            "css": "display: inline-flex; align-items: center; padding: var(--space-s, 0.75rem) var(--space-l, 1.5rem); border-radius: var(--radius, 0.5rem); background: var(--primary, #2563eb); color: var(--white, #fff); font-size: var(--text-m, 1rem); font-weight: 600; text-decoration: none; transition: background 0.2s; &:hover { background: var(--primary-hover, #1e40af); } &:focus-visible { outline: var(--focus-width, 2px) solid var(--focus-color, #2563eb); outline-offset: var(--focus-offset, 2px); }",
            "readonly": false
        }
    }
}
...
> **How local refs are saved.** A `ref` that names a component defined here (by its `id` or map key) is local: the persister creates a `wp_block` post for each component, writes its `blocks`, `properties` and `key` where Etch reads them, and rewrites every instance's `ref` to that post. `"componentId": "<map key>"` is accepted as well. A `ref` that names no component in the payload must be an existing `wp_block` post on the site. Styles used inside `blocks` go in the top-level `styles` map and count as referenced.
