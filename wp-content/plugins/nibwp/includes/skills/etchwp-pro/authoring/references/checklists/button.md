...
- [ ] `border-radius`: `--radius` / `--radius-m` / `--radius-circle` (circle for pill buttons).
- [ ] `background`: `--primary` / `--secondary` for fill; raw transparent for ghost / outline.
- [ ] `color`: `--white` on filled buttons; `--text-dark` on outline / ghost (`--text-light` when the section is dark).
- [ ] `transition` ≤ 250ms.
...
## Behavior
- [ ] `&:hover` — background shift via `--primary-hover` or `filter: brightness(0.95)`.
- [ ] `&:focus-visible` — `outline: var(--focus-width, 2px) solid var(--focus-color, #2563eb); outline-offset: var(--focus-offset, 2px);`. Never `outline: none` without a replacement.
