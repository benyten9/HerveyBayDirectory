...
3. **Walk the HTML tree**, mapping each node:
   ...
   - For full-section artifacts: wrap in `section[data-etch-element="section"]` → `div[data-etch-element="container"]`, then your content. If the source HTML already has this wrapping, reuse it; if not, add it (it's the canonical Etch section scaffold).
...
6. **Substitute raw values with ACSS tokens.** For every CSS value, look up the closest token in [acss-tokens.md](acss-tokens.md) and replace with `var(--token, original-value)`. Examples:
   ...
   - `color: #111` → `color: var(--text-dark, #111)`
...
9. **Submit the payload** to `nibwp/etchwp-pro-html-to-component` — `dry_run: true`, fix what the verdict names, then `dry_run: false`. The ability performs the write; there is no file to place.
...
2. **Extract typography**: heading size (map to the nearest `--h1`…`--h6` or `--text-*` token, measured size as the rem fallback — never `clamp()`), body size (`--text-m` / `--text-l`), weight, line-height.
3. **Extract colors**:
...
Visual fidelity caveat: Etch styles are single-string CSS. Complex layered effects (multi-layer shadows, conic gradients, SVG masks) are fine in CSS but harder to read. Prefer the simplest CSS that matches the design.

---

## 4. Refactor existing JSON (lint mode)

**Input**: an existing payload, or a page's stored block tree, that you've been asked to normalize or bring up to etchwp standards.

### Steps

1. **Read the tree** fully — `nibwp/wp-get-post` for a page, or the component's `wp_block` post.
...
3. **Common fixes** (in order):
   ...
   - **Missing fallback on tokens** — `font-size: var(--text-m);` → `font-size: var(--text-m, 1rem);`. Pick the canonical fallback from [acss-tokens.md](acss-tokens.md).
   - **Raw CSS values without `var()` wrapper** — `padding: 24px` → `padding: var(--space-l, 1.5rem)`. Pick the closest token.
   - **Hardcoded accent colors** — a hex listed in `nibwp_user_defaults` for this brand is OK; otherwise wrap in `var()` with the hex as fallback: `background: #57e9db` → `background: var(--primary, #57e9db)` (or a more appropriate token if there is one).
...
5. **Check every `etch/component` resolves** — each instance carries a `ref` to a component defined in `payload.components` (its `id`) or to a real `wp_block` post, and every component has its tree under `blocks`.
6. **Check `__libraryMeta`** — all six fields present and `slug` equals `sanitize_title(name)`.
7. **Run the checklist.**
...
2. **Identify the component tree.** Map the JSX tree to Etch blocks:
   ...
   - `<img>` → `etch/element` tag="img". `<svg>` → `etch/element` tag="svg" with one `etch/element` child per shape (`path`, `circle`, `rect`, `line`, `polyline`, `polygon`, `g`), attributes copied across (json-schema.md § Inline icons). `etch/svg` only loads an uploaded file by attachment ID; it does not render inline markup.
3. **Convert Tailwind to ACSS-token CSS.** Do NOT keep Tailwind classes. Translate every Tailwind utility to its CSS equivalent, then map to the closest ACSS token with fallback:
   ...
   - `text-gray-500` → `color: var(--text-dark-muted, #6b7280);`
...
|---|---|---|
| `p-1` / `p-2` / `p-3` / `p-4` / `p-6` / `p-8` | `--space-xs` / `--space-xs` / `--space-s` / `--space-m` / `--space-l` / `--space-xl` | `0.25rem` / `0.5rem` / `0.75rem` / `1rem` / `1.5rem` / `3rem` |
...
| `rounded` / `rounded-md` / `rounded-lg` / `rounded-full` | `--radius-xs` / `--radius` / `--radius-m` / `--radius-circle` | `0.25rem` / `0.5rem` / `0.75rem` / `50vw` |
| `text-gray-900` / `text-gray-600` / `text-gray-400` | `--text-dark` / `--text-dark` / `--text-dark-muted` | `#111` / `#374151` / `#6b7280` |
| `bg-white` / `bg-gray-50` / `bg-gray-900` | `--white` / `--neutral-ultra-light` / `--bg-dark` | `#fff` / `#f2f2f2` / `#0f1117` |
...
1. **Measure the visual.** Extract from the image:
   ...
   - **Typography**: heading size (nearest `--h*` / `--text-*` token, measured size as the rem fallback), body size, weight, line-height, font family if identifiable.
...
6. **Add responsive rules.** Even if the image only shows desktop, add sensible collapse behavior: grids → single column below 768px, headings step down one `--h*` / `--text-*` token at the breakpoint, decorative elements hide on small screens.
