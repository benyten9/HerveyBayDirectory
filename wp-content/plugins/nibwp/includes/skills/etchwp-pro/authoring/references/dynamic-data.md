...
Block form (this skill's `etch/condition`) — one `condition` object, plus the same test as a plain expression in `conditionString` (no `{#if}` wrapper; that form is only for inline templates):
```json
{ "blockName": "etch/condition",
  "attrs": { "condition": { "leftHand": "props.showLede", "operator": "isTruthy", "rightHand": null },
             "conditionString": "props.showLede" },
  "innerBlocks": [ /* shown when true */ ] }
```
To combine tests, nest condition objects and join them — there is no list of conditions:
`"condition": { "leftHand": { "leftHand": "props.a", "operator": "isTruthy", "rightHand": null }, "operator": "&&", "rightHand": { "leftHand": "props.b", "operator": "isTruthy", "rightHand": null } }` with `"conditionString": "props.a && props.b"`.
