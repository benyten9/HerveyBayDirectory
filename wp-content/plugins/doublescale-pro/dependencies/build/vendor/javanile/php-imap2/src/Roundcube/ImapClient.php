...
class ImapClient
{
    ...
    public $selected;
    public $data = array();
    public $flags = array('SEEN' => '\\Seen', 'DELETED' => '\\Deleted', 'ANSWERED' => '\\Answered', 'DRAFT' => '\\Draft', 'FLAGGED' => '\\Flagged', 'FORWARDED' => '$Forwarded', 'MDNSENT' => '$MDNSent', '*' => '\\*');
    protected $fp;
    protected $host;
    protected $cmd_tag;
    protected $cmd_num = 0;
     ...
     */
    protected function putLine($string, $endln = \true, $anonymized = \false)
    {
        if (!$this->fp) {
            return \false;
        }
     ...
     */
    protected function putLineC($string, $endln = \true, $anonymized = \false)
    {
        if (!$this->fp) {
            return \false;
        }
            ...
            if ($str) {
                ...
                if (\preg_match("/^\\[([a-z-]+)\\]/i", $str, $m)) {
                    ...
                    if (\preg_match("/^\\[APPENDUID [0-9]+ ([0-9]+)\\]/i", $str, $m)) {
                        ...
                        if (\preg_match("/^\\[COPYUID [0-9]+ ([0-9,:]+) ([0-9,:]+)\\]/i", $str, $m)) {
    ...
    protected function eof()
    {
        ...
        $start = \microtime(\true);
        if (\feof($this->fp) || $this->prefs['timeout'] && \microtime(\true) - $start > $this->prefs['timeout']) {
            $this->closeSocket();
            return \true;
        }
        return \false;
    ...
    protected function authenticate($user, $pass, $type = 'PLAIN')
    {
        if ($type == 'CRAM-MD5' || $type == 'DIGEST-MD5') {
            if ($type == 'DIGEST-MD5' && !\class_exists('DoubleScale\\Pro\\Vendor\\Auth_SASL')) {
                return $this->setError(self::ERROR_BYE, "The Auth_SASL package is required for DIGEST-MD5 authentication");
            }
            ...
            if ($type == 'CRAM-MD5') {
                ...
                if (\strpos($challenge, 'rspauth=') === \false) {
                    return $this->setError(self::ERROR_BAD, "Unexpected response from server to DIGEST-MD5 response");
                }
                $this->putLine('');
            }
            $line = $this->readReply();
            ...
            if ($type == 'GSSAPI') {
                if (!\extension_loaded('krb5')) {
                    return $this->setError(self::ERROR_BYE, "The krb5 extension is required for GSSAPI authentication");
                }
                if (empty($this->prefs['gssapi_cn'])) {
                    return $this->setError(self::ERROR_BYE, "The gssapi_cn parameter is required for GSSAPI authentication");
                }
                if (empty($this->prefs['gssapi_context'])) {
                    return $this->setError(self::ERROR_BYE, "The gssapi_context parameter is required for GSSAPI authentication");
                }
                ...
                try {
                    ...
                    if (!$gssapicontext->wrap(\pack("CCCC", 0x1, 0, 0, 0), $otoken, \true)) {
                ...
                if ($type == 'PLAIN') {
                    ...
                    if ($this->getCapability('SASL-IR')) {
                        list($result, $line) = $this->execute("AUTHENTICATE PLAIN", array($reply), self::COMMAND_RAW_LASTLINE | self::COMMAND_CAPABILITY | self::COMMAND_ANONYMIZED);
                    } else {
                    ...
                    if ($type == 'LOGIN') {
                        ...
                        $this->putLine(\base64_encode($user), \true, \true);
                        $line = \trim($this->readReply());
                        if ($line[0] != '+') {
                            return $this->parseResult($line);
                        }
                        // send result, get reply and process it
                        $this->putLine(\base64_encode($pass), \true, \true);
                        $line = $this->readReply();
                        $result = $this->parseResult($line);
                    } else {
                        if ($type == 'XOAUTH2') {
                            $auth = \base64_encode("user={$user}\x01auth=Bearer {$pass}\x01\x01");
                            $this->putLine($this->nextTag() . " AUTHENTICATE XOAUTH2 {$auth}", \true, \true);
                            $line = \trim($this->readReply());
                            if ($line[0] == '+') {
                                // send empty line
                                $this->putLine('', \true, \true);
                                $line = $this->readReply();
                            }
                            $result = $this->parseResult($line);
                        }
        ...
        if ($result === self::ERROR_OK) {
            ...
            if ($line && \preg_match('/\\[CAPABILITY ([^]]+)\\]/i', $line, $matches)) {
                $this->parseCapability($matches[1], \true);
            }
            return $this->fp;
        }
    ...
    protected function login($user, $password)
    {
        ...
        list($code, $response) = $this->execute('LOGIN', array($this->escape($user), $this->escape($password)), self::COMMAND_CAPABILITY | self::COMMAND_ANONYMIZED);
    ...
    public function getNamespace()
    {
        ...
        $this->prefs['namespace'] = array('personal' => $data[0], 'other' => $data[1], 'shared' => $data[2]);
        return $this->prefs['namespace'];
    }
    /**
    ...
    public function connect($host, $user, $password, $options = array())
    {
        ...
        $token = \json_decode(\base64_decode(\str_replace('_', '/', \str_replace('-', '+', @\explode('.', $password)[0]))), \true);
    ...
    protected function _connect($host)
    {
        ...
        if (!empty($this->prefs['socket_options'])) {
            ...
            $this->fp = \stream_socket_client($host . ':' . $this->prefs['port'], $errno, $errstr, $this->prefs['timeout'], \STREAM_CLIENT_CONNECT, $context);
        } else {
            $this->fp = @\fsockopen($host, $this->prefs['port'], $errno, $errstr, $this->prefs['timeout']);
        }
        if (!$this->fp) {
            $this->setError(self::ERROR_BAD, \sprintf("Could not connect to %s:%d: %s", $host, $this->prefs['port'], $errstr ?: "Unknown reason"));
            return \false;
        }
        if ($this->prefs['timeout'] > 0) {
        ...
        if ($this->prefs['ssl_mode'] == 'tls' && $this->getCapability('STARTTLS')) {
            ...
            if (!\stream_socket_enable_crypto($this->fp, \true, $crypto_method)) {
        ...
        if ($code == self::ERROR_OK) {
            ...
            foreach ($response as $line) {
                if (\preg_match('/^\\* OK \\[/i', $line)) {
                    ...
                    if (\preg_match('/^\\* ([0-9]+) (EXISTS|RECENT|FETCH)/i', $line, $match)) {
                        ...
                        if (\preg_match('/^\\* VANISHED [()EARLIER]*/i', $line, $match)) {
                            $line = \substr($line, \strlen($match[0]));
    ...
    public function status($mailbox, $items = array())
    {
        ...
        list($code, $response) = $this->execute('STATUS', array($this->escape($mailbox), '(' . \implode(' ', $items) . ')'), 0, '/^\\* STATUS /i');
    ...
    {
        return $this->_listMailboxes($ref, $mailbox, \true, $return_opts, null);
    }
    /**
     ...
     *                    is requested, False on error.
     */
    protected function _listMailboxes($ref, $mailbox, $subscribed = \false, $return_opts = array(), $select_opts = array())
    {
        if (!\strlen($mailbox)) {
            $mailbox = '*';
        }
    ...
    public function id($items = array())
    {
        if (\is_array($items) && !empty($items)) {
            foreach ($items as $key => $value) {
                $args[] = $this->escape($key, \true);
                $args[] = $this->escape($value, \true);
            }
        }
        list($code, $response) = $this->execute('ID', array(!empty($args) ? '(' . \implode(' ', (array) $args) . ')' : $this->escape(null)), 0, '/^\\* ID /i');
        if ($code == self::ERROR_OK && $response) {
    ...
    public function sort($mailbox, $field = 'ARRIVAL', $criteria = '', $return_uid = \false, $encoding = 'US-ASCII')
    {
        ...
        if (!\in_array($field, $supported)) {
            return new \Javanile\Imap2\Roundcube\ResultIndex($mailbox);
        }
        if (!$this->select($mailbox)) {
            return new \Javanile\Imap2\Roundcube\ResultIndex($mailbox);
        }
        ...
        if ($old_sel != $mailbox && !$this->data['EXISTS']) {
            return new \Javanile\Imap2\Roundcube\ResultIndex($mailbox, '* SORT');
        }
        ...
        list($code, $response) = $this->execute($return_uid ? 'UID SORT' : 'SORT', array("({$field})", $encoding, $criteria));
        if ($code != self::ERROR_OK) {
            $response = null;
        }
        return new \Javanile\Imap2\Roundcube\ResultIndex($mailbox, $response);
    }
    /**
    ...
    public function thread($mailbox, $algorithm = 'REFERENCES', $criteria = '', $return_uid = \false, $encoding = 'US-ASCII')
    {
        $old_sel = $this->selected;
        if (!$this->select($mailbox)) {
            return new \Javanile\Imap2\Roundcube\ResultThread($mailbox);
        }
        // return empty result when folder is empty and we're just after SELECT
        if ($old_sel != $mailbox && !$this->data['EXISTS']) {
            return new \Javanile\Imap2\Roundcube\ResultThread($mailbox, '* THREAD');
        }
        $encoding = $encoding ? \trim($encoding) : 'US-ASCII';
        $algorithm = $algorithm ? \trim($algorithm) : 'REFERENCES';
        $criteria = $criteria ? 'ALL ' . \trim($criteria) : 'ALL';
        list($code, $response) = $this->execute($return_uid ? 'UID THREAD' : 'THREAD', array($algorithm, $encoding, $criteria));
        if ($code != self::ERROR_OK) {
            $response = null;
        }
        return new \Javanile\Imap2\Roundcube\ResultThread($mailbox, $response);
    }
    /**
    ...
    public function search($mailbox, $criteria, $return_uid = \false, $items = array())
    {
        ...
        // return empty result when folder is empty and we're just after SELECT
        if ($old_sel != $mailbox && !$this->data['EXISTS']) {
            return new \Javanile\Imap2\Roundcube\ResultIndex($mailbox, '* SEARCH');
        }
     ...
     * @return rcube_result_index Response data
     */
    public function index($mailbox, $message_set, $index_field = '', $skip_deleted = \true, $uidfetch = \false, $return_uid = \false)
    {
        $msg_index = $this->fetchHeaderIndex($mailbox, $message_set, $index_field, $skip_deleted, $uidfetch, $return_uid);
        if (!empty($msg_index)) {
            \asort($msg_index);
            // ASC
        ...
        return new \Javanile\Imap2\Roundcube\ResultIndex($mailbox, $msg_index);
    }
    /**
     ...
     */
    public function fetchHeaderIndex($mailbox, $message_set, $index_field = '', $skip_deleted = \true, $uidfetch = \false, $return_uid = \false)
    {
        ...
        if ($mode == 1) {
            ...
            if ($mode == 2) {
                if ($index_field == 'SIZE') {
                    ...
                    if (!$return_uid || $index_field != 'UID') {
                        $fields[] = $index_field;
                    }
                }
            } else {
                if ($mode == 3 && !$skip_deleted) {
                    $fields[] = 'FLAGS';
                } else {
                    if ($mode == 4) {
                        $fields[] = 'INTERNALDATE';
            ...
            if (\preg_match('/^\\* ([0-9]+) FETCH/', $line, $m)) {
                ...
                if ($mode == 1 && $index_field == 'DATE') {
                    if (\preg_match('/BODY\\[HEADER\\.FIELDS \\("*DATE"*\\)\\] (.*)/', $line, $matches)) {
                        ...
                        $value = \trim($value);
                        $result[$id] = \Javanile\Imap2\Roundcube\Utils::strtotime($value);
                    }
                    // non-existent/empty Date: header, use INTERNALDATE
                    if (empty($result[$id])) {
                        if (\preg_match('/INTERNALDATE "([^"]+)"/', $line, $matches)) {
                            $result[$id] = \Javanile\Imap2\Roundcube\Utils::strtotime($matches[1]);
                        } else {
                            $result[$id] = 0;
                        }
                    }
        ...
        } while (!$this->startsWith($line, $key, \true, \true));
        return $result;
    }
    /**
    ...
    public function ID2UID($mailbox, $id)
    {
        ...
        $index = $this->search($mailbox, $id, \true);
        if ($index->count() == 1) {
            $arr = $index->get();
    ...
    protected function modFlag($mailbox, $messages, $flag, $mod = '+')
    {
        ...
        if (!empty($this->data['PERMANENTFLAGS']) && !\in_array($flag, (array) $this->data['PERMANENTFLAGS']) && !\in_array('\\*', (array) $this->data['PERMANENTFLAGS'])) {
            return \false;
        }
        // Clear internal status cache
        if ($flag == 'SEEN') {
        ...
        $result = $this->execute('UID STORE', array($this->compressMessageSet($messages), $mod . 'FLAGS.SILENT', "({$flag})"), self::COMMAND_NORESPONSE);
        return $result == self::ERROR_OK;
    }
    /**
    ...
    public function copy($messages, $from, $to)
    {
        ...
        $result = $this->execute('UID COPY', array($this->compressMessageSet($messages), $this->escape($to)), self::COMMAND_NORESPONSE);
        return $result == self::ERROR_OK;
    }
    /**
    ...
    public function move($messages, $from, $to)
    {
        ...
        if ($this->hasCapability('MOVE')) {
            ...
            $result = $this->execute('UID MOVE', array($this->compressMessageSet($messages), $this->escape($to)), self::COMMAND_NORESPONSE);
            return $result == self::ERROR_OK;
        }
    ...
    public function fetch($mailbox, $message_set, $is_uid = \false, $query_items = array(), $mod_seq = null, $vanished = \false)
    {
        ...
        do {
            ...
            if (\preg_match('/^\\* ([0-9]+) FETCH/', $line, $m)) {
                $id = \intval($m[1]);
                $result[$id] = new \Javanile\Imap2\Roundcube\MessageHeader();
                $result[$id]->id = $id;
                $result[$id]->subject = '';
                        ...
                        if ($name == 'RFC822.SIZE') {
                            ...
                            if ($name == 'RFC822.TEXT') {
                                ...
                                if ($name == 'INTERNALDATE') {
                                    ...
                                    $result[$id]->timestamp = \Javanile\Imap2\Roundcube\Utils::strtotime($value);
                                } else {
                                    if ($name == 'FLAGS') {
                                        if (!empty($value)) {
                                        ...
                                        if ($name == 'MODSEQ') {
                                            ...
                                            if ($name == 'ENVELOPE') {
                                                ...
                                                if ($name == 'BODYSTRUCTURE' || $name == 'BODY' && \count($value) > 2) {
                    ...
                    foreach ($lines as $str) {
                        ...
                        switch ($field) {
                            case 'date':
                                ...
                                $result[$id]->timestamp = \Javanile\Imap2\Roundcube\Utils::strtotime($string);
                                break;
                            case 'to':
        ...
        } while (!$this->startsWith($line, $key, \true));
        return $result;
    }
    /**
    ...
    public function fetchHeaders($mailbox, $message_set, $is_uid = \false, $bodystr = \false, $add_headers = array())
    {
        ...
        $headers = array('DATE', 'FROM', 'TO', 'SUBJECT', 'CONTENT-TYPE', 'CC', 'REPLY-TO', 'LIST-POST', 'DISPOSITION-NOTIFICATION-TO', 'X-PRIORITY');
    ...
    public static function sortHeaders($messages, $field, $flag)
    {
        ...
        foreach ($messages as $key => $headers) {
            switch ($field) {
                ...
                case 'internaldate':
                case 'timestamp':
                    $value = \Javanile\Imap2\Roundcube\Utils::strtotime($headers->{$field});
     ...
     *                    False on failure.
     */
    public function fetchMIMEHeaders($mailbox, $uid, $parts, $mime = \true)
    {
        if (!$this->select($mailbox)) {
            return \false;
        }
    ...
    public function handlePartBody($mailbox, $id, $is_uid = \false, $part = '', $encoding = null, $print = null, $file = null, $formatted = \false, $max_bytes = 0)
    {
        ...
        } while (!$this->startsWith($line, $key, \true) || !$initiated);
        if ($result !== \false) {
            if ($file) {
    ...
    public function setACL($mailbox, $user, $acl)
    {
        ...
        $result = $this->execute('SETACL', array($this->escape($mailbox), $this->escape($user), \strtolower($acl)), self::COMMAND_NORESPONSE);
        return $result == self::ERROR_OK;
    }
    /**
    ...
    {
        $result = $this->execute('DELETEACL', array($this->escape($mailbox), $this->escape($user)), self::COMMAND_NORESPONSE);
        return $result == self::ERROR_OK;
    }
    /**
    ...
    {
        list($code, $response) = $this->execute('LISTRIGHTS', array($this->escape($mailbox), $this->escape($user)), 0, '/^\\* LISTRIGHTS /i');
        if ($code == self::ERROR_OK && $response) {
            ...
            return array('granted' => \str_split($granted), 'optional' => \explode(' ', $optional));
        }
    }
    /**
    ...
    public function setMetadata($mailbox, $entries)
    {
        ...
        foreach ($entries as $name => $value) {
            $entries[$name] = $this->escape($name) . ' ' . $this->escape($value, \true);
        }
        $entries = \implode(' ', $entries);
        $result = $this->execute('SETMETADATA', array($this->escape($mailbox), '(' . $entries . ')'), self::COMMAND_NORESPONSE);
        return $result == self::ERROR_OK;
    }
    /**
    ...
    public function getMetadata($mailbox, $entries, $options = array())
    {
        ...
        list($code, $response) = $this->execute('GETMETADATA', array($this->escape($mailbox), $optlist));
        if ($code == self::ERROR_OK) {
            $result = array();
            $data = $this->tokenizeResponse($response);
            ...
            for ($i = 0, $size = \count($data); $i < $size; $i++) {
                if ($data[$i] === '*' && $data[++$i] === 'METADATA' && \is_string($mbox = $data[++$i]) && \is_array($data[++$i])) {
    ...
    public function setAnnotation($mailbox, $data)
    {
        ...
        foreach ($data as $entry) {
            ...
            $entries[] = \sprintf('%s (%s %s)', $this->escape($entry[0], \true), $this->escape($entry[1], \true), $this->escape($entry[2], \true));
        }
        $entries = \implode(' ', $entries);
        $result = $this->execute('SETANNOTATION', array($this->escape($mailbox), $entries), self::COMMAND_NORESPONSE);
        return $result == self::ERROR_OK;
    }
    /**
    ...
    public function getAnnotation($mailbox, $entries, $attribs)
    {
        ...
        // ANNOTATEMORE drafts before version 08 require quoted parameters
        foreach ($entries as $idx => $name) {
            $entries[$idx] = $this->escape($name, \true);
        }
        $entries = '(' . \implode(' ', $entries) . ')';
        if (!\is_array($attribs)) {
        ...
        foreach ($attribs as $idx => $name) {
            $attribs[$idx] = $this->escape($name, \true);
        }
        $attribs = '(' . \implode(' ', $attribs) . ')';
        list($code, $response) = $this->execute('GETANNOTATION', array($this->escape($mailbox), $entries, $attribs));
        if ($code == self::ERROR_OK) {
            $result = array();
            $data = $this->tokenizeResponse($response);
                    ...
                    if (!empty($attribs)) {
                        for ($x = 0, $len = \count($attribs); $x < $len;) {
                            ...
                            if ($attr == 'value.priv' && $value !== null) {
                                ...
                                if ($attr == 'value.shared' && $value !== null) {
    ...
    public function execute($command, $arguments = array(), $options = 0, $filter = null)
    {
        ...
        // Send command
        if (!$this->putLineC($query, \true, $options & self::COMMAND_ANONYMIZED)) {
        ...
        } while (!$this->startsWith($line, $tag . ' ', \true, \true));
        ...
        // optional CAPABILITY response
        if ($options & self::COMMAND_CAPABILITY && $code == self::ERROR_OK && \preg_match('/\\[CAPABILITY ([^]]+)\\]/i', $line, $matches)) {
            $this->parseCapability($matches[1], \true);
        }
    ...
    protected function clear_mailbox_cache()
    {
        ...
        $keys = array('UIDNEXT', 'UIDVALIDITY', 'HIGHESTMODSEQ', 'NOMODSEQ', 'PERMANENTFLAGS', 'QRESYNC', 'VANISHED', 'READ-WRITE');
        foreach ($keys as $key) {
            unset($this->data[$key]);
        }
    }
