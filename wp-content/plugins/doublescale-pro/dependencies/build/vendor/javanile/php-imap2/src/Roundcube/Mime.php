...
class Mime
{
    ...
    {
        $conf = array('include_bodies' => \true, 'decode_bodies' => \true, 'decode_headers' => \false, 'default_charset' => self::get_charset());
     ...
     */
    static function decode_address_list($input, $max = null, $decode = \true, $fallback = null, $addronly = \false)
    {
        ...
        if (\preg_match_all($re, $input, $matches, \PREG_OFFSET_CAPTURE | \PREG_SET_ORDER)) {
            ...
            foreach ($matches as $idx => $m) {
                ...
                if ($start != $pos) {
                    ...
                    $out .= \Javanile\Imap2\Roundcube\Charset::convert($substr, $default_charset);
                    $start = $pos;
                }
                $start += $length;
                ...
                $out .= \Javanile\Imap2\Roundcube\Charset::convert($text, $charset);
                $tmp = array();
            }
            // add the last part of the input string
            if ($start != \strlen($input)) {
                $out .= \Javanile\Imap2\Roundcube\Charset::convert(\substr($input, $start), $default_charset);
            }
            // return the results
            return $out;
        }
        // no encoding information, use fallback
        return \Javanile\Imap2\Roundcube\Charset::convert($input, $default_charset);
    }
    /**
     * Decode a mime part
     *
     ...
     */
    private static function parse_address_list($str, $decode = \true, $fallback = null)
    {
        // remove any newlines and carriage returns before
        $str = \preg_replace('/\\r?\\n(\\s|\\t)?/', ' ', $str);
        // extract list items, remove comments
        $str = self::explode_header_string(',;', $str, \true);
        $result = array();
        ...
        foreach ($str as $key => $val) {
            ...
            if (\preg_match('/(.*)<(' . $email_rx . ')>$/', $val, $m)) {
                ...
                if (\preg_match('/^(' . $email_rx . ')$/', $val, $m)) {
                    ...
                    if (\preg_match('/(\\s*<MAILER-DAEMON>)$/', $val, $m)) {
        ...
        for ($i = 0; $i < $length; $i++) {
            ...
            if ($quoted) {
                ...
                if ($comment > 0) {
                    ...
                    if (\strpos($separator, $str[$i]) !== \false) {
                        if ($out) {
                            $result[] = $out;
                        }
                        $out = '';
    ...
    public static function unfold_flowed($text, $mark = null, $delsp = \false)
    {
        ...
        foreach ($text as $idx => $line) {
            if ($q = \strspn($line, '>')) {
                ...
                if ($q == $q_level && isset($text[$last]) && $text[$last][\strlen($text[$last]) - 1] == ' ' && !\preg_match('/^>+ {0,1}$/', $text[$last])) {
                    if ($delsp) {
                ...
                if ($line == '-- ') {
                    ...
                    if (isset($text[$last]) && $line && !$q_level && $text[$last] != '-- ' && $text[$last][\strlen($text[$last]) - 1] == ' ') {
                        if ($delsp) {
                            $text[$last] = \substr($text[$last], 0, -1);
                        }
                        $text[$last] .= $line;
     ...
     * @return string Text
     */
    public static function wordwrap($string, $width = 75, $break = "\n", $cut = \false, $charset = null, $wrap_quoted = \true)
    {
    ...
    public static function get_mime_extensions($mimetype = null)
    {
        ...
        $aliases = array('image/gif' => array('gif'), 'image/png' => array('png'), 'image/x-png' => array('png'), 'image/jpeg' => array('jpg', 'jpeg', 'jpe'), 'image/jpg' => array('jpg', 'jpeg', 'jpe'), 'image/pjpeg' => array('jpg', 'jpeg', 'jpe'), 'image/tiff' => array('tif'), 'image/bmp' => array('bmp'), 'image/x-ms-bmp' => array('bmp'), 'message/rfc822' => array('eml'), 'text/x-mail' => array('eml'));
    ...
    public static function image_content_type($data)
    {
        ...
        if (\preg_match('/^\\x89\\x50\\x4E\\x47/', $data)) {
            ...
            if (\preg_match('/^\\x47\\x49\\x46\\x38/', $data)) {
                $type = 'gif';
            } else {
                if (\preg_match('/^\\x00\\x00\\x01\\x00/', $data)) {
                    $type = 'ico';
                }
            }
        }
    ...
    {
        $parts = \Javanile\Imap2\Roundcube\Utils::explode_quoted_string('@', $email);
