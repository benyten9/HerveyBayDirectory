...
class MessageHeader
{
     ...
     */
    public function get($name, $decode = \true)
    {
        ...
        if ($decode) {
            if (\is_array($value)) {
                foreach ($value as $key => $val) {
                    $val = \Javanile\Imap2\Roundcube\Mime::decode_header($val, $this->charset);
                    $value[$key] = \Javanile\Imap2\Roundcube\Charset::clean($val);
                }
            } else {
                $value = \Javanile\Imap2\Roundcube\Mime::decode_header($value, $this->charset);
                $value = \Javanile\Imap2\Roundcube\Charset::clean($value);
            }
        }
        return $value;
    }
    ...
    {
        $obj = new \Javanile\Imap2\Roundcube\MessageHeader();
        foreach ($arr as $k => $v) {
            $obj->set($k, $v);
        }
        return $obj;
