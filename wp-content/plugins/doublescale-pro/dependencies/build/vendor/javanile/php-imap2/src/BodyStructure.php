...
class BodyStructure
{
    ...
    const ENCOTHER = 5;
    /* unknown */
    protected static $body_types = [self::TYPETEXT => "TEXT", self::TYPEMULTIPART => "MULTIPART", self::TYPEMESSAGE => "MESSAGE", self::TYPEAPPLICATION => "APPLICATION", self::TYPEAUDIO => "AUDIO", self::TYPEIMAGE => "IMAGE", self::TYPEVIDEO => "VIDEO", self::TYPEMODEL => "MODEL", self::TYPEOTHER => "X-UNKNOWN"];
    protected static $body_encodings = [self::ENC7BIT => "7BIT", self::ENC8BIT => "8BIT", self::ENCBINARY => "BINARY", self::ENCBASE64 => "BASE64", self::ENCQUOTEDPRINTABLE => "QUOTED-PRINTABLE", self::ENCOTHER => "X-UNKNOWN"];
    ...
    protected static function extractBodyStructure($structure)
    {
        ...
        $body = (object) ["type" => self::TYPEOTHER, "encoding" => self::ENC7BIT, "ifsubtype" => 0, "subtype" => null, "ifdescription" => 0, "description" => null, "ifid" => 0, "id" => null, "lines" => null, "bytes" => null, "ifdisposition" => 0, "disposition" => null, "ifdparameters" => 0, "dparameters" => null, "ifparameters" => 0, "parameters" => null];
        /* multipart body? */
        ...
        if (\is_null($body->disposition)) {
            unset($body->disposition);
        }
        if (\is_null($body->dparameters)) {
            unset($body->dparameters);
        }
        if (\is_null($body->parameters)) {
