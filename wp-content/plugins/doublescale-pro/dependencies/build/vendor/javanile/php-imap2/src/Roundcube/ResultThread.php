...
class ResultThread
{
    ...
    public function count_messages()
    {
        ...
        if (empty($this->raw_data)) {
            ...
            $this->meta['messages'] = 1 + \substr_count($this->raw_data, self::SEPARATOR_ELEMENT) + \substr_count($this->raw_data, self::SEPARATOR_ITEM);
        }
    ...
    {
        $msgid = (int) $msgid;
        $begin = \implode('|', array('^', \preg_quote(self::SEPARATOR_ELEMENT, '/'), \preg_quote(self::SEPARATOR_LEVEL, '/')));
        $end = \implode('|', array('$', \preg_quote(self::SEPARATOR_ELEMENT, '/'), \preg_quote(self::SEPARATOR_ITEM, '/')));
        if (\preg_match("/({$begin}){$msgid}({$end})/", $this->raw_data, $m, $get_index ? \PREG_OFFSET_CAPTURE : null)) {
            if ($get_index) {
                $idx = 0;
                if ($m[0][1]) {
                    $idx = \substr_count($this->raw_data, self::SEPARATOR_ELEMENT, 0, $m[0][1] + 1) + \substr_count($this->raw_data, self::SEPARATOR_ITEM, 0, $m[0][1] + 1);
                }
    ...
    public function get()
    {
        ...
        $regexp = '/(' . \preg_quote(self::SEPARATOR_ELEMENT, '/') . '|' . \preg_quote(self::SEPARATOR_ITEM, '/') . '[0-9]+' . \preg_quote(self::SEPARATOR_LEVEL, '/') . ')/';
