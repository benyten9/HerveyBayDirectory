...
class HeaderInfo
{
    public static function fromMessage($message, $defaultHost)
    {
        ...
        $to = \Javanile\Imap2\Functions::writeAddressFromEnvelope($message->envelope[5]);
        $cc = \Javanile\Imap2\Functions::writeAddressFromEnvelope($message->envelope[6]);
        $from = \Javanile\Imap2\Functions::writeAddressFromEnvelope($message->envelope[2]);
        $sender = \Javanile\Imap2\Functions::writeAddressFromEnvelope($message->envelope[3]);
        if (empty($message->replyto)) {
            $replyTo = $from;
        } else {
            $replyTo = \Javanile\Imap2\Functions::writeAddressFromEnvelope($message->envelope[4]);
        }
        $headerInfo = ['date' => $message->envelope[0], 'Date' => $message->envelope[0], 'subject' => $message->envelope[1], 'Subject' => $message->envelope[1], 'in_reply_to' => $message->envelope[8], 'message_id' => $message->envelope[9], 'references' => $message->references, 'toaddress' => $to, 'to' => self::parseAddressList($to, $defaultHost), 'fromaddress' => $from, 'from' => self::parseAddressList($from, $defaultHost), 'ccaddress' => $cc, 'cc' => self::parseAddressList($cc, $defaultHost), 'reply_toaddress' => $replyTo, 'reply_to' => self::parseAddressList($replyTo, $defaultHost), 'senderaddress' => $sender, 'sender' => self::parseAddressList($sender, $defaultHost), 'Recent' => ' ', 'Unseen' => isset($message->flags['SEEN']) && $message->flags['SEEN'] ? ' ' : 'U', 'Flagged' => empty($message->flags['FLAGGED']) ? ' ' : 'F', 'Answered' => empty($message->flags['ANSWERED']) ? ' ' : 'A', 'Deleted' => empty($message->flags['DELETED']) ? ' ' : 'X', 'Draft' => empty($message->flags['DRAFT']) ? ' ' : 'D', 'Msgno' => \str_pad($message->id, 4, ' ', \STR_PAD_LEFT), 'MailDate' => self::sanitizeMailDate($message->internaldate), 'Size' => \strval($message->size), 'udate' => \strtotime($message->internaldate)];
    ...
    protected static function parseAddressList($address, $defaultHost)
    {
        ...
        foreach ($addressList as $objectEntry) {
            $addressEntry = (object) ['personal' => $objectEntry->personal ?? null, 'mailbox' => @$objectEntry->mailbox, 'host' => @$objectEntry->host];
