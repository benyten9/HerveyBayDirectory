...
class Connection
{
    ...
    public function openMailbox($mailbox)
    {
        ...
        $mailboxParts = \Javanile\Imap2\Functions::parseMailboxString($mailbox);
        $this->host = \Javanile\Imap2\Functions::getHostFromMailbox($mailboxParts);
        $this->port = @$mailboxParts['port'];
        $this->sslMode = \Javanile\Imap2\Functions::getSslModeFromMailbox($mailboxParts);
    ...
    {
        $connection = new \Javanile\Imap2\Connection($mailbox, $user, $password, $flags, $retries, $options);
        $success = $connection->connect();
        if (empty($success)) {
            \Javanile\Imap2\Errors::appendErrorCanNotOpen($mailbox, $connection->getLastError());
            \trigger_error(\Javanile\Imap2\Errors::couldNotOpenStream($mailbox, \debug_backtrace(), 1), \E_USER_WARNING);
            return \false;
        }
        return $connection;
    }
    public static function reopen($imap, $mailbox, $flags = 0, $retries = 0)
    {
        if (!\is_a($imap, \Javanile\Imap2\Connection::class)) {
            return \Javanile\Imap2\Errors::invalidImapConnection(\debug_backtrace(), 1, null);
        }
    ...
    protected function connect()
    {
        ...
        #$client->setDebug(true);
        $success = $client->connect($this->host, $this->user, $this->password, ['port' => $this->port, 'ssl_mode' => $this->sslMode, 'auth_type' => $this->flags & OP_XOAUTH2 ? 'XOAUTH2' : 'CHECK', 'timeout' => -1, 'force_caps' => \false]);
        if (empty($success)) {
            return \false;
        }
    ...
    {
        $mailboxParts = \Javanile\Imap2\Functions::parseMailboxString($this->mailbox);
    ...
    {
        return \is_a($imap, \Javanile\Imap2\Connection::class) && $imap->isConnected();
    }
