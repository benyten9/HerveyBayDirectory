...
class Message
{
    ...
    public static function headerInfo($imap, $messageNum, $fromLength = 0, $subjectLength = 0, $defaultHost = null)
    {
        ...
        $client = $imap->getClient();
        #$client->setDebug(true);
        $messages = $client->fetch($imap->getMailboxName(), $messageNum, \false, ['BODY.PEEK[HEADER.FIELDS (SUBJECT FROM TO CC REPLY-TO DATE SIZE REFERENCES)]', 'ENVELOPE', 'INTERNALDATE', 'UID', 'FLAGS', 'RFC822.SIZE', 'RFC822.HEADER']);
        if (empty($messages)) {
            return \false;
        }
        foreach ($messages as $message) {
            return \Javanile\Imap2\HeaderInfo::fromMessage($message, $defaultHost);
        }
    }
    public static function headers($imap)
    {
        ...
        foreach ($messages as $message) {
            ...
            $header = ' ' . $unseen . $flagged . $answered . $draft . $deleted . ' ' . \str_pad($message->id, 3, ' ', \STR_PAD_LEFT) . ')' . $date[0] . ' ' . \str_pad($from, 20, ' ') . ' ' . \substr($subject, 0, 25) . ' (' . $message->size . ' chars)';
            $headers[] = $header;
        }
        return $headers;
    }
    ...
    public static function fetchBody($imap, $messageNum, $section, $flags = 0)
    {
        ...
        if (empty($messages)) {
            \trigger_error(\Javanile\Imap2\Errors::badMessageNumber(\debug_backtrace(), 1), \E_USER_WARNING);
            return \false;
        }
        if ($section) {
    ...
    public static function fetchStructure($imap, $messageNum, $flags = 0)
    {
        ...
        foreach ($messages as $message) {
            return \Javanile\Imap2\BodyStructure::fromMessage($message);
        }
    }
    ...
    public static function fetchOverview($imap, $sequence, $flags = 0)
    {
        ...
        $client = $imap->getClient();
        #$client->setDebug(true);
        $messages = $client->fetch($imap->getMailboxName(), $sequence, \false, ['BODY[HEADER.FIELDS (SUBJECT FROM TO CC REPLYTO MESSAGEID DATE SIZE REFERENCES)]', 'UID', 'FLAGS', 'INTERNALDATE', 'RFC822.SIZE', 'ENVELOPE', 'RFC822.HEADER']);
        if ($sequence != '*' && \count($messages) < \Javanile\Imap2\Functions::expectedNumberOfMessages($sequence)) {
            return [];
        }
        $overview = [];
        foreach ($messages as $message) {
    ...
    public static function msgno($imap, $messageUid)
    {
        ...
        $client = $imap->getClient();
        $msgNo = \Javanile\Imap2\ImapHelpers::uidToId($imap, $messageUid);
    ...
    public static function uid($imap, $messageNum)
    {
        ...
        $uid = \Javanile\Imap2\ImapHelpers::idToUid($imap, $messageNum);
