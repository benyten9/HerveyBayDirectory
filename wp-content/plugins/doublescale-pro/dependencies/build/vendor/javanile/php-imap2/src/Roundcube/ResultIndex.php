                ...
                if (\preg_match('/^ (E?SEARCH)/i', $data_item, $m)) {
                    ...
                    if (\strtoupper($m[1]) == 'ESEARCH') {
                        ...
                        if (isset($this->params['ALL'])) {
                            $data_item = \implode(self::SEPARATOR_ELEMENT, \Javanile\Imap2\Roundcube\ImapClient::uncompressMessageSet($this->params['ALL']));
                        }
                    }
                    break;
                }
        ...
        if (!empty($this->meta['pos'])) {
            if (isset($this->meta['pos'][$index])) {
                ...
                if (isset($this->meta['pos'][$index - 1])) {
                    $pos = \strpos($this->raw_data, self::SEPARATOR_ELEMENT, $this->meta['pos'][$index - 1] + 1);
                } else {
                    if (isset($this->meta['pos'][$index + 1])) {
                        $pos = \strrpos($this->raw_data, self::SEPARATOR_ELEMENT, $this->meta['pos'][$index + 1] - $this->length() - 1);
                    }
                }
            }
