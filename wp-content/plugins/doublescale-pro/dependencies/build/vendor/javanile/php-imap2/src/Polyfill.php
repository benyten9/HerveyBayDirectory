...
use DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Message;
...
class Polyfill
{
    ...
    public static function rfc822ParseAdrList($string, $defaultHost)
    {
        ...
        return \Javanile\Imap2\Functions::getAddressObjectList($message->getHeader(HeaderConsts::TO)->getAddresses(), $defaultHost);
    }
    /**
     *
     * @param $headers
    ...
    public static function rfc822ParseHeaders($headers, $defaultHost = 'UNKNOWN')
    {
        ...
        return (object) ['date' => $date, 'Date' => $date, 'subject' => $subject, 'Subject' => $subject, 'message_id' => '<' . $message->getHeaderValue(HeaderConsts::MESSAGE_ID) . '>', 'toaddress' => $message->getHeaderValue(HeaderConsts::TO), 'to' => \Javanile\Imap2\Functions::getAddressObjectList($message->getHeader(HeaderConsts::TO)->getAddresses()), 'fromaddress' => $message->getHeaderValue(HeaderConsts::FROM), 'from' => \Javanile\Imap2\Functions::getAddressObjectList($message->getHeader(HeaderConsts::FROM)->getAddresses()), 'reply_toaddress' => $message->getHeaderValue($hasReplyTo ? HeaderConsts::REPLY_TO : HeaderConsts::FROM), 'reply_to' => \Javanile\Imap2\Functions::getAddressObjectList($message->getHeader($hasReplyTo ? HeaderConsts::REPLY_TO : HeaderConsts::FROM)->getAddresses()), 'senderaddress' => $message->getHeaderValue($hasSender ? HeaderConsts::SENDER : HeaderConsts::FROM), 'sender' => \Javanile\Imap2\Functions::getAddressObjectList($message->getHeader($hasSender ? HeaderConsts::SENDER : HeaderConsts::FROM)->getAddresses())];
    }
