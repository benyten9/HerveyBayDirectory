...
class Errors
{
    ...
    {
        $warning = 'Invalid IMAP connection parameter for ' . $backtrace[$depth]['function'] . '() ' . 'at ' . $backtrace[$depth]['file'] . ' on line ' . $backtrace[$depth]['line'] . '. Source code';
    ...
    public static function couldNotOpenStream($mailbox, $backtrace, $depth)
    {
        ...
        return $backtrace[$depth]['function'] . '(): Couldn\'t open stream ' . $mailbox . ' in ' . $backtrace[$depth]['file'] . ' on line ' . $backtrace[$depth]['line'] . '. Source code';
    }
    public static function badMessageNumber($backtrace, $depth)
    {
        if (\Javanile\Imap2\Functions::isBackportCall($backtrace, $depth)) {
            $depth++;
        }
        return $backtrace[$depth]['function'] . '(): Bad message number in ' . $backtrace[$depth]['file'] . ' on line ' . $backtrace[$depth]['line'] . '. Source code';
    }
