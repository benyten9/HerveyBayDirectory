...
class Mailbox
{
    ...
    {
        if (\is_a($imap, \Javanile\Imap2\Connection::class)) {
            $imap->selectMailbox();
            $client = $imap->getClient();
            $status = $client->status($imap->getMailboxName(), ['MESSAGES', 'RECENT']);
            return (object) ['Date' => \date('D, j M Y G:i:s') . ' +0000 (UTC)', 'Driver' => 'imap', 'Mailbox' => $imap->getMailbox(), 'Nmsgs' => \intval($status['MESSAGES']), 'Recent' => \intval($status['RECENT'])];
        } elseif (IMAP2_RETROFIT_MODE && \is_resource($imap) && \get_resource_type($imap) == 'imap') {
            return \imap_check($imap);
        }
        \trigger_error(\Javanile\Imap2\Errors::invalidImapConnection(\debug_backtrace(), 1), \E_USER_WARNING);
        return \false;
    }
    public static function numMsg($imap)
    {
        if (!\is_a($imap, \Javanile\Imap2\Connection::class)) {
            return \Javanile\Imap2\Errors::invalidImapConnection(\debug_backtrace(), 1, \false);
        }
        $imap->selectMailbox();
        $client = $imap->getClient();
    ...
    public static function numRecent($imap)
    {
        if (\is_a($imap, \Javanile\Imap2\Connection::class)) {
            $client = $imap->getClient();
            $imap->selectMailbox();
            return (object) ['Driver' => 'imap', 'Mailbox' => $imap->getMailbox(), 'Nmsgs' => $client->data['EXISTS'], 'Recent' => $client->data['RECENT']];
        }
        return \imap_check($imap);
    }
    public static function status($imap, $mailbox, $flags)
    {
        ...
        $mailboxName = \Javanile\Imap2\Functions::getMailboxName($mailbox);
        $client = $imap->getClient();
        $items = [];
        $statusKeys = ['MESSAGES' => 'messages', 'UNSEEN' => 'unseen', 'RECENT' => 'recent', 'UIDNEXT' => 'uidnext', 'UIDVALIDITY' => 'uidvalidity'];
    ...
    public static function mailboxMsgInfo($imap)
    {
        ...
        $status = $client->status($mailboxName, ['MESSAGES', 'UNSEEN', 'RECENT', 'UIDNEXT', 'UIDVALIDITY']);
        $mailboxInfo = ['Unread' => \intval($status['UNSEEN']), 'Deleted' => 0, 'Nmsgs' => \intval($status['MESSAGES']), 'Size' => 0, 'Date' => \date('D, j M Y G:i:s') . ' +0000 (UTC)', 'Driver' => 'imap', 'Mailbox' => $imap->getMailbox(), 'Recent' => \intval($status['RECENT'])];
    ...
    public static function getMailboxes($imap, $reference, $pattern)
    {
        ...
        $mailboxes = $client->listMailboxes($referenceParts[1], $pattern);
        foreach ($mailboxes as $mailbox) {
            $attributesValue = \Javanile\Imap2\Functions::getListAttributesValue($client->data['LIST'][$mailbox]);
            if ($mailbox == '[Gmail]' && $imap->getHost() == 'imap.gmail.com') {
                $attributesValue = 34;
            }
            $return[] = (object) ['name' => $referenceParts[0] . '}' . $mailbox, 'attributes' => $attributesValue, 'delimiter' => $delimiter];
        }
        return $return;
    }
    public static function createMailbox($imap, $mailbox)
    {
        ...
        if (!$success) {
            \Javanile\Imap2\Errors::appendError($client->getRawLastLine());
        }
        return $success;
    }
    ...
    public static function deleteMailbox($imap, $mailbox)
    {
        ...
        if (!$success && $imap->getRegistryValue('mailbox', $mailbox, 'deleted')) {
            \Javanile\Imap2\Errors::appendError($result[1]);
        } elseif (!$success) {
            \Javanile\Imap2\Errors::appendError("Can't delete mailbox {$mailbox}: no such mailbox");
        } else {
            $imap->setRegistryValue('mailbox', $mailbox, 'deleted', \true);
        }
        return $success;
    }
    /**
