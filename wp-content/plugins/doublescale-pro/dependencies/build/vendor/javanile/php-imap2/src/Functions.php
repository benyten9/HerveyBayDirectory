...
class Functions
{
    ...
    {
        return \Javanile\Imap2\Connection::isValid($imap);
    }
    public static function getAddressObjectList($addressList, $defaultHost = 'UNKNOWN')
    {
        ...
        foreach ($addressList as $toAddress) {
            ...
            $addressObject = (object) ['mailbox' => $email[0], 'host' => $email[1] ?? $defaultHost];
    ...
    {
        return isset($backtrace[$depth + 1]['function']) && \preg_match('/^imap_/', $backtrace[$depth + 1]['function']) && \preg_match('/^imap2_/', $backtrace[$depth]['function']) && \substr($backtrace[$depth + 1]['function'], 4) == \substr($backtrace[$depth]['function'], 5);
    }
