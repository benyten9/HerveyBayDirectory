...
namespace DoubleScale\Pro\Vendor\Sabre\CalDAV\Backend;

use DoubleScale\Pro\Vendor\Sabre\CalDAV;
use DoubleScale\Pro\Vendor\Sabre\DAV;
use DoubleScale\Pro\Vendor\Sabre\DAV\Exception\Forbidden;
use DoubleScale\Pro\Vendor\Sabre\DAV\PropPatch;
use DoubleScale\Pro\Vendor\Sabre\DAV\Xml\Element\Sharee;
use DoubleScale\Pro\Vendor\Sabre\VObject;
/**
 * PDO CalDAV backend.
 *
...
SQL
        ...
        while ($row = $stmt->fetch(\PDO::FETCH_ASSOC)) {
            ...
            if ($row['access'] > 1) {
                ...
                // the future.
                $calendar['read-only'] = \DoubleScale\Pro\Vendor\Sabre\DAV\Sharing\Plugin::ACCESS_READ === (int) $row['access'];
            }
    ...
    public function deleteCalendar($calendarId)
    {
        ...
        if (\DoubleScale\Pro\Vendor\Sabre\DAV\Sharing\Plugin::ACCESS_SHAREDOWNER === $access) {
            /**
    ...
    public function createCalendarObject($calendarId, $objectUri, $calendarData)
    {
        ...
        $stmt->execute([$calendarId, $objectUri, $calendarData, \time(), $extraData['etag'], $extraData['size'], $extraData['componentType'], $extraData['firstOccurence'], $extraData['lastOccurence'], $extraData['uid']]);
    ...
    public function updateCalendarObject($calendarId, $objectUri, $calendarData)
    {
        ...
        $stmt->execute([$calendarData, \time(), $extraData['etag'], $extraData['size'], $extraData['componentType'], $extraData['firstOccurence'], $extraData['lastOccurence'], $extraData['uid'], $calendarId, $objectUri]);
    ...
    protected function getDenormalizedData($calendarData)
    {
        ...
        if (!$componentType) {
            throw new \DoubleScale\Pro\Vendor\Sabre\DAV\Exception\BadRequest('Calendar objects must have a VJOURNAL, VEVENT or VTODO component');
        }
...
SQL;
    ...
    public function createSubscription($principalUri, $uri, array $properties)
    {
        ...
        $values = [':principaluri' => $principalUri, ':uri' => $uri, ':source' => $properties['{http://calendarserver.org/ns/}source']->getHref(), ':lastmodified' => \time()];
        foreach ($this->subscriptionPropertyMap as $xmlName => $dbName) {
            if (isset($properties[$xmlName])) {
                $values[':' . $dbName] = $properties[$xmlName];
                $fieldNames[] = $dbName;
    ...
    public function createSchedulingObject($principalUri, $objectUri, $objectData)
    {
        ...
        $stmt->execute([$principalUri, $objectData, $objectUri, \time(), \md5($objectData), \strlen($objectData)]);
    }
    /**
...
INSERT INTO ' . $this->calendarInstancesTableName . '
    ...
    FROM ' . $this->calendarInstancesTableName . ' WHERE id = ?');
        foreach ($sharees as $sharee) {
            if (\DoubleScale\Pro\Vendor\Sabre\DAV\Sharing\Plugin::ACCESS_NOACCESS === $sharee->access) {
            ...
            if (\is_null($sharee->principal)) {
                ...
                $sharee->inviteStatus = \DoubleScale\Pro\Vendor\Sabre\DAV\Sharing\Plugin::INVITE_INVALID;
            } else {
                // Because sabre/dav does not yet have an invitation system,
                // every invite is automatically accepted for now.
                $sharee->inviteStatus = \DoubleScale\Pro\Vendor\Sabre\DAV\Sharing\Plugin::INVITE_ACCEPTED;
            }
            ...
            $insertStmt->execute([$calendarId, $sharee->principal, $sharee->access, \DoubleScale\Pro\Vendor\Sabre\DAV\UUIDUtil::getUUID(), $sharee->href, isset($sharee->properties['{DAV:}displayname']) ? $sharee->properties['{DAV:}displayname'] : null, $sharee->inviteStatus ?: \DoubleScale\Pro\Vendor\Sabre\DAV\Sharing\Plugin::INVITE_NORESPONSE, $instanceId]);
        }
    }
    /**
...
SQL;
        ...
        while ($row = $stmt->fetch(\PDO::FETCH_ASSOC)) {
            $result[] = new Sharee([
                'href' => isset($row['share_href']) ? $row['share_href'] : \DoubleScale\Pro\Vendor\Sabre\HTTP\encodePath($row['principaluri']),
