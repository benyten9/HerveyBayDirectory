...
namespace DoubleScale\Pro\Vendor\Sabre\DAV;

use Psr\Log\LoggerAwareInterface;
use Psr\Log\LoggerAwareTrait;
use Psr\Log\LoggerInterface;
use Psr\Log\NullLogger;
use DoubleScale\Pro\Vendor\Sabre\Event\EmitterInterface;
use DoubleScale\Pro\Vendor\Sabre\Event\WildcardEmitterTrait;
...
class Server implements LoggerAwareInterface, EmitterInterface
{
     ...
     * @var array
     */
    public $resourceTypeMapping = ['DoubleScale\\Pro\\Vendor\\Sabre\\DAV\\ICollection' => '{DAV:}collection'];
    /**
    ...
    public function calculateUri($uri)
    {
        ...
        if (0 === \strpos($uri, $baseUri)) {
            return \trim(HTTP\decodePath(\substr($uri, \strlen($baseUri))), '/');
            ...
            if ($nodeExists) {
                ...
                if ('*' === $ifNoneMatch) {
                    ...
                    foreach ($ifNoneMatch as $ifNoneMatchItem) {
                        // Stripping any extra spaces
                        $ifNoneMatchItem = \trim($ifNoneMatchItem, ' ');
