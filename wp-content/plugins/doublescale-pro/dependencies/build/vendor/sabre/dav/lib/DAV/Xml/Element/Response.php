...
namespace DoubleScale\Pro\Vendor\Sabre\DAV\Xml\Element;

use DoubleScale\Pro\Vendor\Sabre\Xml\Element;
use DoubleScale\Pro\Vendor\Sabre\Xml\Reader;
use DoubleScale\Pro\Vendor\Sabre\Xml\Writer;
/**
...
class Response implements Element
{
    ...
    public function xmlSerialize(Writer $writer)
    {
         ...
         */
        $writer->writeElement('{DAV:}href', $writer->contextUri . \DoubleScale\Pro\Vendor\Sabre\HTTP\encodePath($this->getHref()));
        $empty = \true;
        ...
        foreach ($this->getResponseProperties() as $status => $properties) {
            ...
            $writer->writeElement('{DAV:}status', 'HTTP/1.1 ' . $status . ' ' . \DoubleScale\Pro\Vendor\Sabre\HTTP\Response::$statusCodes[$status]);
            $writer->endElement();
            // {DAV:}propstat
        }
        ...
        if ($empty) {
            if (null !== $httpStatus) {
                $writer->writeElement('{DAV:}status', 'HTTP/1.1 ' . $httpStatus . ' ' . \DoubleScale\Pro\Vendor\Sabre\HTTP\Response::$statusCodes[$httpStatus]);
            } else {
                /*
                 ...
                 * no properties.
                 */
                $writer->writeElement('{DAV:}propstat', ['{DAV:}prop' => [], '{DAV:}status' => 'HTTP/1.1 418 ' . \DoubleScale\Pro\Vendor\Sabre\HTTP\Response::$statusCodes[418]]);
            }
        }
    }
    /**
    ...
    {
        $reader->pushContext();
        $reader->elementMap['{DAV:}propstat'] = 'DoubleScale\\Pro\\Vendor\\Sabre\\Xml\\Element\\KeyValue';
