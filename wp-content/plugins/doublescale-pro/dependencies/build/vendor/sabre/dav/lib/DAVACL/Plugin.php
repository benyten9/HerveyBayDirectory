...
namespace DoubleScale\Pro\Vendor\Sabre\DAVACL;

use DoubleScale\Pro\Vendor\Sabre\DAV;
use DoubleScale\Pro\Vendor\Sabre\DAV\Exception\BadRequest;
use DoubleScale\Pro\Vendor\Sabre\DAV\Exception\Forbidden;
use DoubleScale\Pro\Vendor\Sabre\DAV\Exception\NotAuthenticated;
use DoubleScale\Pro\Vendor\Sabre\DAV\Exception\NotFound;
use DoubleScale\Pro\Vendor\Sabre\DAV\INode;
use DoubleScale\Pro\Vendor\Sabre\DAV\Xml\Property\Href;
use DoubleScale\Pro\Vendor\Sabre\DAVACL\Exception\NeedPrivileges;
use DoubleScale\Pro\Vendor\Sabre\HTTP\RequestInterface;
use DoubleScale\Pro\Vendor\Sabre\HTTP\ResponseInterface;
use DoubleScale\Pro\Vendor\Sabre\Uri;
/**
 * SabreDAV ACL Plugin.
 *
...
class Plugin extends DAV\ServerPlugin
{
     ...
     * @var bool
     */
    public $allowUnauthenticatedAccess = \true;
    /**
    ...
    {
        return ['{DAV:}expand-property', '{DAV:}principal-match', '{DAV:}principal-property-search', '{DAV:}principal-search-property-set'];
    }
    /**
     ...
     * @return bool
     */
    public function checkPrivileges($uri, $privileges, $recursion = self::R_PARENT, $throwExceptions = \true)
    {
        ...
        if ($failed) {
            if ($this->allowUnauthenticatedAccess && \is_null($this->getCurrentUserPrincipal())) {
                ...
                $authPlugin->challenge($this->server->httpRequest, $this->server->httpResponse);
    ...
    public function getCurrentUserPrincipals()
    {
        ...
        return \array_merge([$currentUser], $this->getPrincipalMembership($currentUser));
    }
    /**
     ...
     * @var array
     */
    protected $defaultAcl = [['principal' => '{DAV:}authenticated', 'protected' => \true, 'privilege' => '{DAV:}all']];
    /**
    ...
    public function principalMatchesPrincipal($checkPrincipal, $currentPrincipal = null)
    {
        ...
        return \in_array($checkPrincipal, $this->getPrincipalMembership($currentPrincipal));
    }
    /**
     ...
     * @return array
     */
    public final function getFlatPrivilegeSet($node)
    {
        $privs = ['abstract' => \false, 'aggregates' => $this->getSupportedPrivilegeSet($node)];
        $fpsTraverse = null;
        $fpsTraverse = function ($privName, $privInfo, $concrete, &$flat) use(&$fpsTraverse) {
            $myPriv = ['privilege' => $privName, 'abstract' => isset($privInfo['abstract']) && $privInfo['abstract'], 'aggregates' => [], 'concrete' => isset($privInfo['abstract']) && $privInfo['abstract'] ? $concrete : $privName];
    ...
    public function getAcl($node)
    {
        ...
        foreach ($this->adminPrincipals as $adminPrincipal) {
            $acl[] = ['principal' => $adminPrincipal, 'privilege' => '{DAV:}all', 'protected' => \true];
        }
        return $acl;
    }
    /**
    ...
    public function initialize(DAV\Server $server)
    {
        ...
        // class.
        $server->xml->elementMap['{DAV:}group-member-set'] = 'DoubleScale\\Pro\\Vendor\\Sabre\\DAV\\Xml\\Property\\Href';
        $server->xml->elementMap['{DAV:}acl'] = 'DoubleScale\\Pro\\Vendor\\Sabre\\DAVACL\\Xml\\Property\\Acl';
        $server->xml->elementMap['{DAV:}acl-principal-prop-set'] = 'DoubleScale\\Pro\\Vendor\\Sabre\\DAVACL\\Xml\\Request\\AclPrincipalPropSetReport';
        $server->xml->elementMap['{DAV:}expand-property'] = 'DoubleScale\\Pro\\Vendor\\Sabre\\DAVACL\\Xml\\Request\\ExpandPropertyReport';
        $server->xml->elementMap['{DAV:}principal-property-search'] = 'DoubleScale\\Pro\\Vendor\\Sabre\\DAVACL\\Xml\\Request\\PrincipalPropertySearchReport';
        $server->xml->elementMap['{DAV:}principal-search-property-set'] = 'DoubleScale\\Pro\\Vendor\\Sabre\\DAVACL\\Xml\\Request\\PrincipalSearchPropertySetReport';
        $server->xml->elementMap['{DAV:}principal-match'] = 'DoubleScale\\Pro\\Vendor\\Sabre\\DAVACL\\Xml\\Request\\PrincipalMatchReport';
    }
    /* {{{ Event handlers */
    /**
    ...
    public function propPatch($path, DAV\PropPatch $propPatch)
    {
        $propPatch->handle('{DAV:}group-member-set', function ($value) use($path) {
            if (\is_null($value)) {
                ...
                $memberSet = \array_map([$this->server, 'calculateUri'], $value->getHrefs());
            } else {
    ...
    protected function principalMatchReport($path, Xml\Request\PrincipalMatchReport $report)
    {
        ...
        if (Xml\Request\PrincipalMatchReport::SELF === $report->type) {
            ...
            $candidates = $this->server->getPropertiesForPath($path, [$report->principalProperty], 1);
            foreach ($candidates as $candidate) {
                ...
                foreach ($hrefs->getHrefs() as $href) {
                    if (\in_array(\trim($href, '/'), $currentPrincipals)) {
        ...
        foreach ($result as $item) {
            ...
            $responses[] = new DAV\Xml\Element\Response($item, $properties, '200');
        }
        $this->server->httpResponse->setHeader('Content-Type', 'application/xml; charset=utf-8');
        $this->server->httpResponse->setStatus(207);
        $this->server->httpResponse->setBody($this->server->xml->write('{DAV:}multistatus', $responses, $this->server->getBaseUri()));
    }
    /**
    ...
    {
        return ['name' => $this->getPluginName(), 'description' => 'Adds support for WebDAV ACL (rfc3744)', 'link' => 'http://sabre.io/dav/acl/'];
    }
}
