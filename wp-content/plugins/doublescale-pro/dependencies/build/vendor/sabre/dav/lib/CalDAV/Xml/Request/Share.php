...
namespace DoubleScale\Pro\Vendor\Sabre\CalDAV\Xml\Request;

use DoubleScale\Pro\Vendor\Sabre\CalDAV\Plugin;
...
class Share implements XmlDeserializable
{
    ...
    {
        $elems = $reader->parseGetElements(['{' . Plugin::NS_CALENDARSERVER . '}set' => 'DoubleScale\\Pro\\Vendor\\Sabre\\Xml\\Element\\KeyValue', '{' . Plugin::NS_CALENDARSERVER . '}remove' => 'DoubleScale\\Pro\\Vendor\\Sabre\\Xml\\Element\\KeyValue']);
        $sharees = [];
        foreach ($elems as $elem) {
            switch ($elem['name']) {
                case '{' . Plugin::NS_CALENDARSERVER . '}set':
                    ...
                    $access = \array_key_exists('{' . Plugin::NS_CALENDARSERVER . '}read-write', $sharee) ? \DoubleScale\Pro\Vendor\Sabre\DAV\Sharing\Plugin::ACCESS_READWRITE : \DoubleScale\Pro\Vendor\Sabre\DAV\Sharing\Plugin::ACCESS_READ;
                ...
                case '{' . Plugin::NS_CALENDARSERVER . '}remove':
                    $sharees[] = new Sharee(['href' => $elem['value']['{DAV:}href'], 'access' => \DoubleScale\Pro\Vendor\Sabre\DAV\Sharing\Plugin::ACCESS_NOACCESS]);
                    break;
            }
        }
