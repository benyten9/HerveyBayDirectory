...
use DoubleScale\Pro\Vendor\Sabre\DAV\Exception\ReportNotSupported;
...
class Plugin extends DAV\ServerPlugin
{
    ...
    public function initialize(DAV\Server $server)
    {
        ...
        $server->xml->elementMap['{' . self::NS_CARDDAV . '}addressbook-query'] = 'DoubleScale\\Pro\\Vendor\\Sabre\\CardDAV\\Xml\\Request\\AddressBookQueryReport';
        $server->xml->elementMap['{' . self::NS_CARDDAV . '}addressbook-multiget'] = 'DoubleScale\\Pro\\Vendor\\Sabre\\CardDAV\\Xml\\Request\\AddressBookMultiGetReport';
        ...
        $server->xml->elementMap['{http://calendarserver.org/ns/}me-card'] = 'DoubleScale\\Pro\\Vendor\\Sabre\\DAV\\Xml\\Property\\Href';
        $this->server = $server;
    }
    /**
    ...
    public function getSupportedReportSet($uri)
    {
        ...
        if ($node instanceof IAddressBook || $node instanceof ICard) {
            return ['{' . self::NS_CARDDAV . '}addressbook-multiget', '{' . self::NS_CARDDAV . '}addressbook-query'];
        }
        return [];
    }
    /**
    ...
    protected function validateVCard(&$data, &$modified)
    {
        ...
        if ($warningMessage) {
            $this->server->httpResponse->setHeader('X-Sabre-Ew-Gross', 'vCard validation warning: ' . $warningMessage);
    ...
    protected function addressbookQueryReport($report)
    {
        ...
        foreach ($validNodes as $validNode) {
            ...
            if (isset($props[200]['{' . self::NS_CARDDAV . '}address-data'])) {
                $props[200]['{' . self::NS_CARDDAV . '}address-data'] = $this->convertVCard($props[200]['{' . self::NS_CARDDAV . '}address-data'], $vcardType, $report->addressDataProperties);
            }
            $result[] = $props;
        }
        $prefer = $this->server->getHTTPPrefer();
    ...
    {
        return ['name' => $this->getPluginName(), 'description' => 'Adds support for CardDAV (rfc6352)', 'link' => 'http://sabre.io/dav/carddav/'];
    }
}
