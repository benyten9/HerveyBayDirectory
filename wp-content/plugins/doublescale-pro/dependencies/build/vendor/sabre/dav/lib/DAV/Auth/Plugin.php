...
namespace DoubleScale\Pro\Vendor\Sabre\DAV\Auth;
...
class Plugin extends ServerPlugin
{
    ...
    {
        if (!$this->backends) {
            throw new \DoubleScale\Pro\Vendor\Sabre\DAV\Exception('No authentication backends were configured on this server.');
        }
        $reasons = [];
        foreach ($this->backends as $backend) {
            $result = $backend->check($request, $response);
            if (!\is_array($result) || 2 !== \count($result) || !\is_bool($result[0]) || !\is_string($result[1])) {
                throw new \DoubleScale\Pro\Vendor\Sabre\DAV\Exception('The authentication backend did not return a correct value from the check() method.');
            }
            if ($result[0]) {
