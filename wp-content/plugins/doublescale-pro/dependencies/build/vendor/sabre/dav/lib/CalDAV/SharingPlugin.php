...
class SharingPlugin extends DAV\ServerPlugin
{
    ...
    public function initialize(DAV\Server $server)
    {
        ...
        $this->server->xml->elementMap['{' . Plugin::NS_CALENDARSERVER . '}share'] = 'DoubleScale\\Pro\\Vendor\\Sabre\\CalDAV\\Xml\\Request\\Share';
        $this->server->xml->elementMap['{' . Plugin::NS_CALENDARSERVER . '}invite-reply'] = 'DoubleScale\\Pro\\Vendor\\Sabre\\CalDAV\\Xml\\Request\\InviteReply';
    ...
    public function propFindLate(DAV\PropFind $propFind, DAV\INode $node)
    {
        if ($node instanceof ISharedCalendar) {
            ...
            if ($rt = $propFind->get('{DAV:}resourcetype')) {
                switch ($shareAccess) {
                    case \DoubleScale\Pro\Vendor\Sabre\DAV\Sharing\Plugin::ACCESS_SHAREDOWNER:
                        $rt->add('{' . Plugin::NS_CALENDARSERVER . '}shared-owner');
                        break;
                    case \DoubleScale\Pro\Vendor\Sabre\DAV\Sharing\Plugin::ACCESS_READ:
                    case \DoubleScale\Pro\Vendor\Sabre\DAV\Sharing\Plugin::ACCESS_READWRITE:
    ...
    public function propPatch($path, DAV\PropPatch $propPatch)
    {
        ...
        if (\DoubleScale\Pro\Vendor\Sabre\DAV\Sharing\Plugin::ACCESS_SHAREDOWNER === $node->getShareAccess() || \DoubleScale\Pro\Vendor\Sabre\DAV\Sharing\Plugin::ACCESS_NOTSHARED === $node->getShareAccess()) {
