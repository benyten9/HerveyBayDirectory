...
namespace DoubleScale\Pro\Vendor\Sabre\HTTP;
...
function toDate(\DateTime $dateTime) : string
{
    ...
    return $dateTime->format('DoubleScale\\Pro\\Vendor\\D, d M Y H:i:s \\G\\M\\T');
}
/**
...
function negotiateContentType($acceptHeaderValue, array $availableOptions)
{
    ...
    $proposals = \array_map('DoubleScale\\Pro\\Vendor\\Sabre\\HTTP\\parseMimeType', \explode(',', $acceptHeaderValue));
    // Ensuring array keys are reset.
    $availableOptions = \array_values($availableOptions);
    $options = \array_map('DoubleScale\\Pro\\Vendor\\Sabre\\HTTP\\parseMimeType', $availableOptions);
    $lastQuality = 0;
    $lastSpecificity = 0;
    $lastOptionIndex = 0;
    $lastChoice = null;
...
function parseMimeType(string $str) : array
{
    ...
    $mimeType = \trim(\array_shift($parts));
    if ('*' === $mimeType) {
        $mimeType = '*/*';
    }
