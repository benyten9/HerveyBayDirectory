...
namespace DoubleScale\Pro\Vendor\Sabre\VObject\Property\ICalendar;
...
use DoubleScale\Pro\Vendor\Sabre\VObject\TimeZoneUtil;
/**
 * DateTime property.
 *
...
class DateTime extends Property
{
    ...
    public function setDateTimes(array $dt, $isFloating = \false)
    {
        ...
        if ($this->hasTime()) {
            ...
            foreach ($dt as $d) {
                if ($isFloating) {
                    $values[] = $d->format('DoubleScale\\Pro\\Vendor\\Ymd\\THis');
                    continue;
                }
                if (\is_null($tz)) {
                    $tz = $d->getTimeZone();
                ...
                if ($isUtc) {
                    $values[] = $d->format('DoubleScale\\Pro\\Vendor\\Ymd\\THis\\Z');
                } else {
                    $values[] = $d->format('DoubleScale\\Pro\\Vendor\\Ymd\\THis');
                }
            }
