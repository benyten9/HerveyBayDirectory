...
use DoubleScale\Pro\Vendor\Stripe\StripeObject;
abstract class Util
{
    ...
    {
        $types = 'v1' === $apiMode ? \DoubleScale\Pro\Vendor\Stripe\Util\ObjectTypes::mapping : \DoubleScale\Pro\Vendor\Stripe\Util\ObjectTypes::v2Mapping;
        if (self::isList($resp)) {
            $mapped = [];
            foreach ($resp as $i) {
        ...
        if (\is_array($resp)) {
            if (isset($resp['object']) && \is_string($resp['object']) && isset($types[$resp['object']])) {
                ...
                if ('v2' === $apiMode && 'v2.core.event' === $resp['object']) {
                    $eventTypes = \DoubleScale\Pro\Vendor\Stripe\Util\EventTypes::thinEventMapping;
                    if (\array_key_exists('type', $resp) && \array_key_exists($resp['type'], $eventTypes)) {
                        $class = $eventTypes[$resp['type']];
                    } else {
                        $class = \DoubleScale\Pro\Vendor\Stripe\V2\Event::class;
                    }
                }
                ...
                $class = \DoubleScale\Pro\Vendor\Stripe\V2\Collection::class;
            } else {
                $class = \DoubleScale\Pro\Vendor\Stripe\StripeObject::class;
            }
        ...
        foreach ($properties as $key => $property) {
            if (\array_key_exists($property->getName(), $json)) {
                if ('related_object' === $property->getName()) {
                    $related_object = new \DoubleScale\Pro\Vendor\Stripe\RelatedObject();
                    ...
                    $reason = new \DoubleScale\Pro\Vendor\Stripe\Reason();
    ...
    {
        if ($h instanceof \DoubleScale\Pro\Vendor\Stripe\ApiResource) {
            return $h->id;
        }
        if (static::isList($h)) {
            $results = [];
