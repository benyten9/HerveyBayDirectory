...
namespace DoubleScale\Pro\Vendor\Stripe\Service;

class OAuthService extends \DoubleScale\Pro\Vendor\Stripe\Service\AbstractService
{
    /**
    ...
    public function authorizeUrl($params = null, $opts = null)
    {
        ...
        $query = \DoubleScale\Pro\Vendor\Stripe\Util\Util::encodeParameters($params);
    ...
    private function _parseOpts($opts)
    {
        if (\is_array($opts)) {
            if (\array_key_exists('connect_base', $opts)) {
                ...
                throw new \DoubleScale\Pro\Vendor\Stripe\Exception\InvalidArgumentException('Use `api_base`, not `connect_base`');
            }
        }
        return \DoubleScale\Pro\Vendor\Stripe\Util\RequestOptions::parse($opts);
    }
    /**
