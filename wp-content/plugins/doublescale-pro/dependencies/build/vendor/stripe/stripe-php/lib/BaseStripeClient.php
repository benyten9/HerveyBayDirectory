...
namespace DoubleScale\Pro\Vendor\Stripe;

use DoubleScale\Pro\Vendor\Stripe\Util\Util;
class BaseStripeClient implements StripeClientInterface, StripeStreamingClientInterface
{
    ...
    public function __construct($config = [])
    {
        if (\is_string($config)) {
            ...
            throw new \DoubleScale\Pro\Vendor\Stripe\Exception\InvalidArgumentException('$config must be a string or an array');
        }
        $config = \array_merge(self::DEFAULT_CONFIG, $config);
        $this->validateConfig($config);
        $this->config = $config;
        $this->defaultOpts = \DoubleScale\Pro\Vendor\Stripe\Util\RequestOptions::parse(['stripe_account' => $config['stripe_account'], 'stripe_context' => $config['stripe_context'], 'stripe_version' => $config['stripe_version']]);
    }
    /**
    ...
    public function request($method, $path, $params, $opts)
    {
        ...
        $apiMode = \DoubleScale\Pro\Vendor\Stripe\Util\Util::getApiMode($path);
        $opts = $defaultRequestOpts->merge($opts, \true);
        $baseUrl = $opts->apiBase ?: $this->getApiBase();
        $requestor = new \DoubleScale\Pro\Vendor\Stripe\ApiRequestor($this->apiKeyForRequest($opts), $baseUrl, $this->getAppInfo());
        list($response, $opts->apiKey) = $requestor->request($method, $path, $params, $opts->headers, $apiMode, ['stripe_client']);
        $opts->discardNonPersistentHeaders();
        $obj = \DoubleScale\Pro\Vendor\Stripe\Util\Util::convertToStripeObject($response->json, $opts, $apiMode);
        if (\is_array($obj)) {
            // Edge case for v2 endpoints that return empty/void response
            // Example: client->v2->billing->meterEventStream->create
            $obj = new \DoubleScale\Pro\Vendor\Stripe\StripeObject();
        }
    ...
    public function rawRequest($method, $path, $params = null, $opts = [])
    {
        ...
        $baseUrl = $opts->apiBase ?: $this->getApiBase();
        $requestor = new \DoubleScale\Pro\Vendor\Stripe\ApiRequestor($this->apiKeyForRequest($opts), $baseUrl);
    ...
    public function requestCollection($method, $path, $params, $opts)
    {
        ...
        $apiMode = \DoubleScale\Pro\Vendor\Stripe\Util\Util::getApiMode($path);
        if ('v1' === $apiMode) {
            if (!$obj instanceof \DoubleScale\Pro\Vendor\Stripe\Collection) {
                $received_class = \get_class($obj);
                $msg = "Expected to receive `Stripe\\Collection` object from Stripe API. Instead received `{$received_class}`.";
                throw new \DoubleScale\Pro\Vendor\Stripe\Exception\UnexpectedValueException($msg);
            }
            $obj->setFilters($params);
        } else {
            if (!$obj instanceof \DoubleScale\Pro\Vendor\Stripe\V2\Collection) {
                $received_class = \get_class($obj);
    ...
    {
        $obj = $this->request($method, $path, $params, $opts);
        if (!$obj instanceof \DoubleScale\Pro\Vendor\Stripe\SearchResult) {
            $received_class = \get_class($obj);
    ...
    private function apiKeyForRequest($opts)
    {
        ...
        if (null === $apiKey) {
            ...
            throw new \DoubleScale\Pro\Vendor\Stripe\Exception\AuthenticationException($msg);
        }
        return $apiKey;
    }
    /**
    ...
    private function validateConfig($config)
    {
        ...
        if (null !== $config['api_key'] && !\is_string($config['api_key'])) {
            throw new \DoubleScale\Pro\Vendor\Stripe\Exception\InvalidArgumentException('api_key must be null or a string');
        }
        if (null !== $config['api_key'] && '' === $config['api_key']) {
            $msg = 'api_key cannot be the empty string';
            throw new \DoubleScale\Pro\Vendor\Stripe\Exception\InvalidArgumentException($msg);
        }
        ...
        if (null !== $config['client_id'] && !\is_string($config['client_id'])) {
            throw new \DoubleScale\Pro\Vendor\Stripe\Exception\InvalidArgumentException('client_id must be null or a string');
        }
        // stripe_account
        if (null !== $config['stripe_account'] && !\is_string($config['stripe_account'])) {
            throw new \DoubleScale\Pro\Vendor\Stripe\Exception\InvalidArgumentException('stripe_account must be null or a string');
        }
        // stripe_context
        if (null !== $config['stripe_context'] && !\is_string($config['stripe_context'])) {
            throw new \DoubleScale\Pro\Vendor\Stripe\Exception\InvalidArgumentException('stripe_context must be null or a string');
        }
        // stripe_version
        if (null !== $config['stripe_version'] && !\is_string($config['stripe_version'])) {
            throw new \DoubleScale\Pro\Vendor\Stripe\Exception\InvalidArgumentException('stripe_version must be null or a string');
        }
        // api_base
        if (!\is_string($config['api_base'])) {
            throw new \DoubleScale\Pro\Vendor\Stripe\Exception\InvalidArgumentException('api_base must be a string');
        }
        // connect_base
        if (!\is_string($config['connect_base'])) {
            throw new \DoubleScale\Pro\Vendor\Stripe\Exception\InvalidArgumentException('connect_base must be a string');
        }
        // files_base
        if (!\is_string($config['files_base'])) {
            throw new \DoubleScale\Pro\Vendor\Stripe\Exception\InvalidArgumentException('files_base must be a string');
        }
        // app info
        if (null !== $config['app_info'] && !\is_array($config['app_info'])) {
            throw new \DoubleScale\Pro\Vendor\Stripe\Exception\InvalidArgumentException('app_info must be an array');
        }
        ...
        if (!empty($extraConfigKeys)) {
            ...
            throw new \DoubleScale\Pro\Vendor\Stripe\Exception\InvalidArgumentException('Found unknown key(s) in configuration array: ' . $invalidKeys);
        }
    }
    /**
    ...
    {
        return \DoubleScale\Pro\Vendor\Stripe\Util\Util::convertToStripeObject(\json_decode($json, \true), [], $apiMode);
    }
    /**
    ...
    public function parseThinEvent($payload, $sigHeader, $secret, $tolerance = Webhook::DEFAULT_TOLERANCE)
    {
        ...
        try {
            return Util::json_decode_thin_event_object($eventData, 'DoubleScale\\Pro\\Vendor\\Stripe\\ThinEvent');
        } catch (\ReflectionException $e) {
            // Fail gracefully
            return new \DoubleScale\Pro\Vendor\Stripe\ThinEvent();
        }
    }
}
