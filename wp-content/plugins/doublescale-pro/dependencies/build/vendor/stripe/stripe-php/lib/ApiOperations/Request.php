...
namespace DoubleScale\Pro\Vendor\Stripe\ApiOperations;
...
trait Request
{
    ...
    protected static function _validateParams($params = null)
    {
        if ($params && !\is_array($params)) {
            ...
            throw new \DoubleScale\Pro\Vendor\Stripe\Exception\InvalidArgumentException($message);
        }
    }
    /**
    ...
    protected static function _requestPage($url, $resultClass, $params = null, $options = null, $usage = [])
    {
        ...
        if (!$obj instanceof $resultClass) {
            throw new \DoubleScale\Pro\Vendor\Stripe\Exception\UnexpectedValueException('Expected type ' . $resultClass . ', got "' . \get_class($obj) . '" instead.');
        }
    ...
    {
        $opts = \DoubleScale\Pro\Vendor\Stripe\Util\RequestOptions::parse($options);
        $baseUrl = isset($opts->apiBase) ? $opts->apiBase : static::baseUrl();
        $requestor = new \DoubleScale\Pro\Vendor\Stripe\ApiRequestor($opts->apiKey, $baseUrl);
