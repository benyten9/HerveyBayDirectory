...
class PrivacyHelper extends AbstractHelper
{
    ...
    public function __construct(IMimePartFactory $mimePartFactory, IUUEncodedPartFactory $uuEncodedPartFactory, GenericHelper $genericHelper, MultipartHelper $multipartHelper)
    {
    ...
    public function setMessageAsMultipartSigned(IMessage $message, $micalg, $protocol)
    {
        if (\strcasecmp($message->getContentType(), 'multipart/signed') !== 0) {
            ...
            $message->setRawHeader(HeaderConsts::CONTENT_TYPE, "multipart/signed;\r\n\tboundary=\"{$boundary}\";\r\n\tmicalg=\"{$micalg}\"; protocol=\"{$protocol}\"");
        }
    ...
    public function setSignature(IMessage $message, $body)
    {
        ...
        $signedPart->setRawHeader(HeaderConsts::CONTENT_TYPE, $message->getHeaderParameter(HeaderConsts::CONTENT_TYPE, 'protocol'));
    ...
    public function overwrite8bitContentEncoding(IMessage $message)
    {
        ...
        foreach ($parts as $part) {
            ...
            $part->setRawHeader(HeaderConsts::CONTENT_TRANSFER_ENCODING, $contentType === 'text/plain' || $contentType === 'text/html' ? 'quoted-printable' : 'base64');
        }
    }
    /**
