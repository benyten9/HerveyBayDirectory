...
class MessagePartStream implements SplObserver, StreamInterface
{
    ...
    private function getCharsetDecoratorForStream(StreamInterface $stream) : StreamInterface
    {
        ...
        if (!empty($charset)) {
            $stream = $this->streamFactory->newCharsetStream($stream, $charset, MailMimeParser::DEFAULT_CHARSET);
        }
        return $stream;
    }
    /**
    ...
    private function getTransferEncodingDecoratorForStream(StreamInterface $stream) : StreamInterface
    {
        ...
        switch ($encoding) {
            case 'quoted-printable':
                ...
                break;
            case 'base64':
                $decorator = $this->streamFactory->newBase64Stream($this->streamFactory->newChunkSplitStream($stream));
                break;
            case 'x-uuencode':
