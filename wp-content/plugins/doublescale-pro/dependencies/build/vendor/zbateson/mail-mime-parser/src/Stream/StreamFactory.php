 ...
 */
namespace DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Stream;

use Psr\Http\Message\StreamInterface;
use DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Message\IMessagePart;
use DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Parser\PartBuilder;
use DoubleScale\Pro\Vendor\ZBateson\StreamDecorators\Base64Stream;
use DoubleScale\Pro\Vendor\ZBateson\StreamDecorators\CharsetStream;
use DoubleScale\Pro\Vendor\ZBateson\StreamDecorators\ChunkSplitStream;
use DoubleScale\Pro\Vendor\ZBateson\StreamDecorators\NonClosingStream;
use DoubleScale\Pro\Vendor\ZBateson\StreamDecorators\PregReplaceFilterStream;
use DoubleScale\Pro\Vendor\ZBateson\StreamDecorators\QuotedPrintableStream;
use DoubleScale\Pro\Vendor\ZBateson\StreamDecorators\SeekingLimitStream;
use DoubleScale\Pro\Vendor\ZBateson\StreamDecorators\UUStream;
/**
...
class StreamFactory
{
    ...
    {
        return $this->newLimitStream($part->getStream(), $part->getStreamPartLength(), $part->getStreamPartStartPos());
    }
    /**
    ...
    public function getLimitedContentStream(PartBuilder $part)
    {
        ...
        if ($length !== 0) {
            return $this->newLimitStream($part->getStream(), $part->getStreamContentLength(), $part->getStreamContentStartPos());
        }
        return null;
    }
    /**
    ...
    {
        return new SeekingLimitStream($this->newNonClosingStream($stream), $length, $start);
    }
    /**
    ...
    {
        return new Base64Stream(new PregReplaceFilterStream($stream, '/[^a-zA-Z0-9\\/\\+=]/', ''));
    }
    /**
