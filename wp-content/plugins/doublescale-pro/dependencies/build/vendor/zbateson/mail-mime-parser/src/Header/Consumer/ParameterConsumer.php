...
use ArrayObject;
use DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Header\IHeaderPart;
use DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Header\Part\MimeLiteralPart;
use DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Header\Part\SplitParameterToken;
use DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Header\Part\Token;
/**
...
class ParameterConsumer extends GenericConsumer
{
    ...
    private function getPartFor(string $strName, string $strValue, ArrayObject $splitParts)
    {
        if ($strName === '') {
            ...
            return $this->addToSplitPart($splitParts, $matches[1], $strValue, (int) $matches[2], $matches[2] === '' || !empty($matches[3]));
        }
    ...
    private function finalizeParameterParts(ArrayObject $combined) : array
    {
        foreach ($combined as $key => $part) {
            if ($part instanceof SplitParameterToken) {
                $combined[$key] = $this->partFactory->newParameterPart($part->getName(), $part->getValue(), $part->getLanguage());
            }
        }
