 ...
 */
namespace DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Header\Consumer;

use DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Header\Part\AddressGroupPart;
use DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Header\Part\AddressPart;
use DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Header\Part\CommentPart;
use DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Header\Part\LiteralPart;
/**
...
class AddressConsumer extends AbstractConsumer
{
    ...
    {
        return [$this->consumerService->getAddressGroupConsumer(), $this->consumerService->getAddressEmailConsumer(), $this->consumerService->getCommentConsumer(), $this->consumerService->getQuotedStringConsumer()];
    }
    /**
    ...
    protected function processParts(array $parts) : array
    {
        ...
        foreach ($parts as $part) {
            if ($part instanceof AddressGroupPart) {
                return [$this->partFactory->newAddressGroupPart($part->getAddresses(), $strName)];
