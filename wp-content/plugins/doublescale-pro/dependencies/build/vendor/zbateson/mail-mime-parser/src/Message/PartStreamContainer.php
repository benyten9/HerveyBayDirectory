...
class PartStreamContainer
{
    ...
    {
        return $fromCharset !== $this->charset['from'] || $toCharset !== $this->charset['to'];
    }
    /**
    ...
    protected function attachCharsetFilter(string $fromCharset, string $toCharset) : self
    {
        if ($this->charsetStream !== null) {
            $this->charsetStream = new CachingStream($this->streamFactory->newCharsetStream($this->charsetStream, $fromCharset, $toCharset));
    ...
    {
        $this->charset = ['from' => null, 'to' => null, 'filter' => null];
    ...
    {
        $this->encoding = ['type' => null, 'filter' => null];
        $this->charset = ['from' => null, 'to' => null, 'filter' => null];
    ...
    public function getContentStream(?string $transferEncoding, ?string $fromCharset, ?string $toCharset)
    {
        ...
        if ($this->charsetStream === null || $this->isTransferEncodingFilterChanged($transferEncoding) || $this->isCharsetFilterChanged($fromCharset, $toCharset)) {
            if ($this->charsetStream === null || $this->isTransferEncodingFilterChanged($transferEncoding)) {
                $this->reset();
    ...
    public function getBinaryContentStream(?string $transferEncoding = null) : ?StreamInterface
    {
        ...
        if ($this->decodedStream === null || $this->isTransferEncodingFilterChanged($transferEncoding)) {
            $this->reset();
            $this->attachTransferEncodingFilter($transferEncoding);
        }
        $this->decodedStream->rewind();
