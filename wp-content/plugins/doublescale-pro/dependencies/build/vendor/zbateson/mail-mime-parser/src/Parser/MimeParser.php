...
use DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Parser\Proxy\ParserMessageProxyFactory;
use DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Parser\Proxy\ParserMimePartProxy;
use DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Parser\Proxy\ParserMimePartProxyFactory;
...
class MimeParser extends AbstractParser
{
    ...
    public function __construct(ParserMessageProxyFactory $parserMessageProxyFactory, ParserMimePartProxyFactory $parserMimePartProxyFactory, PartBuilderFactory $partBuilderFactory, PartHeaderContainerFactory $partHeaderContainerFactory, HeaderParser $headerParser)
    {
    ...
    private function createPart(ParserMimePartProxy $parent, PartHeaderContainer $headerContainer, PartBuilder $child)
    {
        if (!$parent->isEndBoundaryFound()) {
            $this->headerParser->parse($child->getMessageResourceHandle(), $headerContainer);
