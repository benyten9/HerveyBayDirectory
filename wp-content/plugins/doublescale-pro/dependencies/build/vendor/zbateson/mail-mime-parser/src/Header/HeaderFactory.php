...
class HeaderFactory
{
     ...
     */
    protected $types = [\DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Header\AddressHeader::class => ['from', 'to', 'cc', 'bcc', 'sender', 'replyto', 'resentfrom', 'resentto', 'resentcc', 'resentbcc', 'resentreplyto', 'returnpath', 'deliveredto'], \DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Header\DateHeader::class => ['date', 'resentdate', 'deliverydate', 'expires', 'expirydate', 'replyby'], \DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Header\ParameterHeader::class => ['contenttype', 'contentdisposition', 'receivedspf', 'authenticationresults', 'dkimsignature', 'autocrypt'], \DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Header\SubjectHeader::class => ['subject'], \DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Header\IdHeader::class => ['messageid', 'contentid', 'inreplyto', 'references'], \DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Header\ReceivedHeader::class => ['received']];
    /**
     * @var string Defines the generic IHeader type to use for headers that
     *      aren't mapped in $types
     */
    protected $genericType = \DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Header\GenericHeader::class;
    /**
    ...
    {
        if (\is_a($iHeaderClass, 'DoubleScale\\Pro\\Vendor\\ZBateson\\MailMimeParser\\Header\\MimeEncodedHeader', \true)) {
            return new $iHeaderClass($this->mimeLiteralPartFactory, $this->consumerService, $name, $value);
        }
