...
use DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Header\Part\HeaderPartFactory;
...
abstract class AbstractConsumer
{
     ...
     */
    protected abstract function getSubConsumers() : array;
    /**
     ...
     */
    protected abstract function getTokenSeparators() : array;
    /**
    ...
    {
        return \preg_split($this->getTokenSplitPattern(), $rawValue, -1, \PREG_SPLIT_DELIM_CAPTURE | \PREG_SPLIT_NO_EMPTY);
    }
    /**
     ...
     */
    protected abstract function isStartToken(string $token) : bool;
    /**
     ...
     * @param string $token The current token
     */
    protected abstract function isEndToken(string $token) : bool;
    /**
    ...
    protected function getConsumerTokenParts(Iterator $tokens) : array
    {
        ...
        foreach ($subConsumers as $consumer) {
            if ($consumer->isStartToken($token)) {
                $this->advanceToNextToken($tokens, \true);
    ...
    protected function getTokenParts(Iterator $tokens) : array
    {
        ...
        if (\strlen($token) === 2 && $token[0] === '\\') {
            return [$this->getPartForToken(\substr($token, 1), \true)];
        }
