 ...
 */
namespace DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Header\Consumer\Received;

use DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Header\Consumer\ConsumerService;
use DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Header\Consumer\GenericConsumer;
...
class GenericReceivedConsumer extends GenericConsumer
{
    ...
    {
        return ['\\s+', '(\\A\\s*|\\s+)(?i)' . \preg_quote($this->getPartName(), '/') . '(?-i)(?=\\s+)'];
    }
    /**
    ...
    protected function processParts(array $parts) : array
    {
        ...
        foreach ($filtered as $part) {
            if ($part instanceof CommentPart) {
                $ret[] = $part;
                continue;
                // getValue() is empty anyway, but for clarity...
            }
