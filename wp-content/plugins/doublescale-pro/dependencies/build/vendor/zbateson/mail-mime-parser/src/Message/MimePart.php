 ...
 */
namespace DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Message;

use DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Header\HeaderConsts;
use DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Header\IHeader;
use DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Header\ParameterHeader;
use DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\IMessage;
use DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\MailMimeParser;
/**
...
class MimePart extends MultiPart implements IMimePart
{
    ...
    public function __construct(?IMimePart $parent = null, ?PartStreamContainer $streamContainer = null, ?PartHeaderContainer $headerContainer = null, ?PartChildrenContainer $partChildrenContainer = null)
    {
        $setStream = \false;
        $di = MailMimeParser::getDependencyContainer();
        if ($streamContainer === null || $headerContainer === null || $partChildrenContainer === null) {
            $headerContainer = $di[\DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Message\PartHeaderContainer::class];
            $streamContainer = $di[\DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Message\PartStreamContainer::class];
            $partChildrenContainer = $di[\DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Message\PartChildrenContainer::class];
            $setStream = \true;
        }
        parent::__construct($parent, $streamContainer, $partChildrenContainer);
        if ($setStream) {
            $streamFactory = $di[\DoubleScale\Pro\Vendor\ZBateson\MailMimeParser\Stream\StreamFactory::class];
    ...
    {
        return $this->getHeaderParameter(HeaderConsts::CONTENT_DISPOSITION, 'filename', $this->getHeaderParameter(HeaderConsts::CONTENT_TYPE, 'name'));
    }
    /**
     * Returns true.
     *
    ...
    public function isMultiPart()
    {
        ...
        return (bool) \preg_match('~multipart/.*~i', $this->getContentType());
    }
    /**
    ...
    {
        static $translated = ['x-uue' => 'x-uuencode', 'uue' => 'x-uuencode', 'uuencode' => 'x-uuencode'];
