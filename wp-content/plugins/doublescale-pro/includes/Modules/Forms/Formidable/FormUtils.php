...
namespace DoubleScale\Pro\Modules\Forms\Formidable;
...
class FormUtils
{
	...
	public static function prepare_name_field($field, $field_id)
	{
		...
		$format = $field->field_options['format'] ?? 'first-last';
		if ('first-middle-last' === $format || ! empty($field->field_options['middle'] ?? '')) {
	...
	{
		return 'formidable_' . $field_index;
	}
}
