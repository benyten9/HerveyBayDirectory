...
namespace DoubleScale\Pro\Modules\Integrations\Keap;
...
class App {
	...
	public function maybe_add_settings() {
		$state = $_GET['state'] ?? '';
		if ( $state !== 'doublescale-keap' && $state !== 'ds-keap' ) {
			return;
		}
		...
		if ( ! $account['success'] ) {
			echo esc_html__( 'Error, Cannot get account details!', 'doublescale');
			exit;
		}
