...
namespace DoubleScale\Pro\Modules\Automations\Actions\Lifterlms;
	...
	public function process_action( AutomationModel $automation, AutomationStepModel $step, AutomationContactModel $automation_contact ) {
		...
		if ( ! $user ) {
			doublescale_get_logger()->info(
				__( 'User not found for LifterLMS course removal', 'doublescale'),
				array(
		...
		if ( ! $course_id ) {
			doublescale_get_logger()->info(
				__( 'Course ID not configured for LifterLMS removal action', 'doublescale'),
				array(
		...
		if ( ! function_exists( 'llms_get_student' ) ) {
			doublescale_get_logger()->error(
				__( 'LifterLMS plugin is not active. Cannot remove user from course.', 'doublescale'),
				array(
		...
		$result = $student->unenroll( $course_id, 'doublescale_automation_' . $automation->id, 'cancelled' );

		if ( ! $result ) {
			doublescale_get_logger()->info(
				__( 'User could not be removed from the LifterLMS course. Maybe user is not enrolled.', 'doublescale'),
				array(
					'code'          => 'lifterlms_unenrollment_failed',
					'user_id'       => $user->ID,
					'course_id'     => $course_id,
					'automation_id' => $automation->id,
					'step_id'       => $step->id,
		...
		doublescale_get_logger()->info(
			__( 'User successfully removed from LifterLMS course', 'doublescale'),
			array(
				'code'          => 'lifterlms_course_unenrolled',
				'user_id'       => $user->ID,
				'course_id'     => $course_id,
				'automation_id' => $automation->id,
				'step_id'       => $step->id,
