	...
	public function process_action(AutomationModel $automation, AutomationStepModel $step, AutomationContactModel $automation_contact)
	{
		...
		if (! $user) {
			doublescale_get_logger()->info(
				__('User not found for LearnDash course enrollment', 'doublescale'),
				array(
		...
		if (! $course_id) {
			doublescale_get_logger()->info(
				__('Course ID not configured for LearnDash enrollment action', 'doublescale'),
				array(
		...
		if (! function_exists('ld_update_course_access')) {
			doublescale_get_logger()->error(
				__('LearnDash plugin is not active. Cannot add user to course.', 'doublescale'),
				array(
		...
		doublescale_get_logger()->info(
			__('User successfully added to LearnDash course', 'doublescale'),
			array(
