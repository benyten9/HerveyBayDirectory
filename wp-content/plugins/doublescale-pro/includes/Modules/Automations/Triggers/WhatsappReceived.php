...
/**
 * WhatsappReceived class
 */
class WhatsappReceived extends Trigger {
	...
	public function load_hooks() {
		add_action( 'doublescale_whatsapp_received', array( $this, 'handle_whatsapp_received' ), 10, 4 );
	}

	/**
	 * Handle WhatsApp received event
	 *
	 * This method is called when a WhatsApp message is received via webhook.
	 * It structures the data for automation processing and merge tag access.
	 *
	 * @since 1.0.0
	 *
	 * @param \DoubleScale\Modules\Contacts\Models\ContactModel                $contact  Contact who sent the WhatsApp.
	 ...
	 * @param array                                         $whatsapp_data {
	 *     WhatsApp message data from the provider.
	 *
	 *     @type string   $from_number  Sender's phone number (e.g., '+1234567890').
	 *     @type string   $to_number    Your business WhatsApp number.
	 *     @type string   $message_body The text content of the message.
	 *     @type string   $message_id   Meta/Provider message ID for tracking.
	 *     @type string[] $media_urls   Array of media attachment URLs.
	 * }
	 *
	 * @return void
	 */
	public function handle_whatsapp_received( $contact, $activity, $tracking, $whatsapp_data ) {
		/*
		 ...
		 *
		 * Merge tags access data via: $automation_contact->get_data('whatsapp_data')
	...
	public function is_processable( AutomationModel $automation, $args ) {
		...
		if ( ! empty( $message_contains ) ) {
			$message_body = $args['data']['whatsapp_data']['message_body'] ?? '';
