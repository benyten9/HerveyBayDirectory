...
use DirectoristPricingPlan\DI\Annotation\Inject;
use DirectoristPricingPlan\DI\Annotation\Injectable;
use DirectoristPricingPlan\DI\Definition\Exception\InvalidAnnotation;
use DirectoristPricingPlan\DI\Definition\ObjectDefinition;
use DirectoristPricingPlan\DI\Definition\ObjectDefinition\MethodInjection;
use DirectoristPricingPlan\DI\Definition\ObjectDefinition\PropertyInjection;
use DirectoristPricingPlan\DI\Definition\Reference;
use DirectoristPricingPlan\Doctrine\Common\Annotations\AnnotationRegistry;
use DirectoristPricingPlan\Doctrine\Common\Annotations\Reader;
use DirectoristPricingPlan\Doctrine\Common\Annotations\SimpleAnnotationReader;
use InvalidArgumentException;
use DirectoristPricingPlan\PhpDocReader\PhpDocReader;
use ReflectionClass;
...
class AnnotationBasedAutowiring implements DefinitionSource, Autowiring
{
    ...
    private function readProperty(ReflectionProperty $property, ObjectDefinition $definition, $classname = null)
    {
        ...
        $annotation = $this->getAnnotationReader()->getPropertyAnnotation($property, 'DirectoristPricingPlan\\DI\\Annotation\\Inject');
    ...
    private function getMethodInjection(ReflectionMethod $method)
    {
        // Look for @Inject annotation
        try {
            $annotation = $this->getAnnotationReader()->getMethodAnnotation($method, 'DirectoristPricingPlan\\DI\\Annotation\\Inject');
    ...
    public function getAnnotationReader()
    {
        if ($this->annotationReader === null) {
            ...
            $this->annotationReader->addNamespace('DirectoristPricingPlan\\DI\\Annotation');
        }
    ...
    private function readInjectableAnnotation(ReflectionClass $class, ObjectDefinition $definition)
    {
        try {
            ...
            $annotation = $this->getAnnotationReader()->getClassAnnotation($class, 'DirectoristPricingPlan\\DI\\Annotation\\Injectable');
