...
namespace DirectoristStripe\Stripe;

use DirectoristStripe\Stripe\Util\Util;
class BaseStripeClient implements StripeClientInterface, StripeStreamingClientInterface
{
    ...
    const DEFAULT_CONFIG = [
        ...
        'stripe_account' => null,
        'stripe_context' => null,
        'stripe_version' => \DirectoristStripe\Stripe\Util\ApiVersion::CURRENT,
    ...
    public function __construct($config = [])
    {
        ...
        $this->config = $config;
        $this->defaultOpts = \DirectoristStripe\Stripe\Util\RequestOptions::parse(['stripe_account' => $config['stripe_account'], 'stripe_context' => $config['stripe_context'], 'stripe_version' => $config['stripe_version'], 'max_network_retries' => $config['max_network_retries']]);
    }
    /**
    ...
    public function parseThinEvent($payload, $sigHeader, $secret, $tolerance = Webhook::DEFAULT_TOLERANCE)
    {
        ...
        try {
            return Util::json_decode_thin_event_object($eventData, 'DirectoristStripe\\Stripe\\ThinEvent');
