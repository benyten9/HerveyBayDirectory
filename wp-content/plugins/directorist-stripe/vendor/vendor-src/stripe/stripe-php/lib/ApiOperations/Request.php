...
namespace DirectoristStripe\Stripe\ApiOperations;
...
trait Request
{
    ...
    protected static function _validateParams($params = null)
    {
        if ($params && !\is_array($params)) {
            ...
            throw new \DirectoristStripe\Stripe\Exception\InvalidArgumentException($message);
        }
    }
    /**
    ...
    protected static function _requestPage($url, $resultClass, $params = null, $options = null, $usage = [])
    {
        ...
        $obj = \DirectoristStripe\Stripe\Util\Util::convertToStripeObject($response->json, $opts);
        if (!$obj instanceof $resultClass) {
            throw new \DirectoristStripe\Stripe\Exception\UnexpectedValueException('Expected type ' . $resultClass . ', got "' . \get_class($obj) . '" instead.');
        }
    ...
    {
        $opts = \DirectoristStripe\Stripe\Util\RequestOptions::parse($options);
        $baseUrl = isset($opts->apiBase) ? $opts->apiBase : static::baseUrl();
        $requestor = new \DirectoristStripe\Stripe\ApiRequestor($opts->apiKey, $baseUrl);
