...
use DirectoristStripe\DI\Annotation\Inject;
use DirectoristStripe\DI\Annotation\Injectable;
use DirectoristStripe\DI\Definition\Exception\InvalidAnnotation;
use DirectoristStripe\DI\Definition\ObjectDefinition;
use DirectoristStripe\DI\Definition\ObjectDefinition\MethodInjection;
use DirectoristStripe\DI\Definition\ObjectDefinition\PropertyInjection;
use DirectoristStripe\DI\Definition\Reference;
use DirectoristStripe\Doctrine\Common\Annotations\AnnotationRegistry;
use DirectoristStripe\Doctrine\Common\Annotations\Reader;
use DirectoristStripe\Doctrine\Common\Annotations\SimpleAnnotationReader;
use InvalidArgumentException;
use DirectoristStripe\PhpDocReader\PhpDocReader;
use ReflectionClass;
...
class AnnotationBasedAutowiring implements DefinitionSource, Autowiring
{
    ...
    private function readProperty(ReflectionProperty $property, ObjectDefinition $definition, $classname = null)
    {
        ...
        $annotation = $this->getAnnotationReader()->getPropertyAnnotation($property, 'DirectoristStripe\\DI\\Annotation\\Inject');
    ...
    private function getMethodInjection(ReflectionMethod $method)
    {
        // Look for @Inject annotation
        try {
            $annotation = $this->getAnnotationReader()->getMethodAnnotation($method, 'DirectoristStripe\\DI\\Annotation\\Inject');
    ...
    public function getAnnotationReader()
    {
        if ($this->annotationReader === null) {
            ...
            $this->annotationReader->addNamespace('DirectoristStripe\\DI\\Annotation');
        }
    ...
    private function readInjectableAnnotation(ReflectionClass $class, ObjectDefinition $definition)
    {
        try {
            ...
            $annotation = $this->getAnnotationReader()->getClassAnnotation($class, 'DirectoristStripe\\DI\\Annotation\\Injectable');
