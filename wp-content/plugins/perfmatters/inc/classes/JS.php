...
class JS
{
	...
	public static function queue()
	{
		...
		//inline scripts filter
		self::$data['inline'] = apply_filters('perfmatters_inline_scripts', array());
	...
	public static function optimize($html)
	{
		...
		foreach($matches[0] as $i => $tag) {
			...
			//inline scripts
			if(!empty(self::$run['inline']) && !empty($atts_array_new['src']) && Utilities::match_in_array($tag, self::$data['inline'])) {
				if(self::inline_script($html, $tag, $atts_array_new)) {
					continue;
				}
			}
	...
	//swap a script tag with an inline script tag
	private static function inline_script(&$html, $tag, $atts_array)
	{
		//get local script file path
		$file = Utilities::get_file_path($atts_array['src'] ?? '');
		...
		//drop src-related attributes that no longer apply
		unset($atts_array['src'], $atts_array['integrity'], $atts_array['crossorigin']);

		$new_atts_string = Utilities::get_atts_string($atts_array);
		$new_tag = sprintf('<script%1$s>', $new_atts_string ? ' ' . $new_atts_string : '') . $content . '</script>';
		$html = str_replace($tag, $new_tag, $html);
