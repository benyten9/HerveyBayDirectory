...
class HBL_Single_Location_V2 extends Widget_Base {

	use Query_Handler;
	use Card_Renderer;

	public function get_name() {
		return 'hbl-single-location-v2';
	}

	public function get_title() {
		return esc_html__( 'HBL Single Location V2', 'hbl' );
	}
	...
	private function get_current_term() {
		...
		if ( $atbdp_loc ) {
			$term = get_term_by( 'slug', $atbdp_loc, 'at_biz_dir-location' );
		...
		foreach ( $path_parts as $index => $part ) {
			if ( in_array( $part, array( 'single-location', 'location', 'listing-location' ), true ) && isset( $path_parts[ $index + 1 ] ) ) {
				$term = get_term_by( 'slug', $path_parts[ $index + 1 ], 'at_biz_dir-location' );
				if ( $term && ! is_wp_error( $term ) ) {
					return $term;
				}
			}
		...
		if ( isset( $_GET['location'] ) ) {
			$term = get_term_by( 'slug', sanitize_text_field( $_GET['location'] ), 'at_biz_dir-location' );
			if ( $term && ! is_wp_error( $term ) ) {
				return $term;
			}
		}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'show_term_header',
			array(
				'label'        => esc_html__( 'Show Location Header', 'hbl' ),
				'type'         => Controls_Manager::SWITCHER,
				'return_value' => 'yes',
				'default'      => 'yes',
			)
		...
		$this->add_control(
			'fallback_logo',
			array( 'label' => esc_html__( 'Default Logo', 'hbl' ), 'type' => Controls_Manager::MEDIA )
		);
		$this->end_controls_section();
		...
		$tags_repeater->add_control(
			'search_text',
			array(
				'label'   => esc_html__( 'Display Text', 'hbl' ),
		...
		$more_repeater = new \Elementor\Repeater();
		$more_repeater->add_control(
			'filter_text',
			array(
				'label'   => esc_html__( 'Display Text', 'hbl' ),
				'type'    => Controls_Manager::TEXT,
		...
		$this->add_control(
			...
			array(
				...
				'type'        => Controls_Manager::REPEATER,
				'fields'      => $more_repeater->get_controls(),
	...
	protected function render() {
		...
		if ( $current_term ) {
			$query_settings['location_filter_type'] = 'specific';
			$query_settings['location']             = array( $current_term->term_id );
		...
		?>
		<div class="hbl-v2-widget hbl-v2-location-widget"
			...
			<?php if ( ! $current_term ) : ?>
				<div class="hbl-v2-notice">
					<p><?php esc_html_e( 'No location detected. Please navigate to a location page.', 'hbl' ); ?></p>
				</div>
			<?php else : ?>
		...
		if ( $show_popular ) :
			...
			if ( empty( $popular_items ) ) {
				...
				if ( ! is_wp_error( $auto_tags ) && ! empty( $auto_tags ) ) {
					foreach ( $auto_tags as $auto_tag ) {
						$popular_items[] = array(
							'search_text' => $auto_tag->name,
							'search_tag'  => $auto_tag->term_id,
						);
					}
				}
			}
				...
				<div class="hbl-v2-popular-searches-section">
					<div class="hbl-v2-popular-searches-buttons">
						<?php foreach ( $popular_items as $item ) :
							$tag_id   = ! empty( $item['search_tag'] ) ? $item['search_tag'] : 0;
							$tag_name = ! empty( $item['search_text'] ) ? $item['search_text'] : '';
							if ( ! $tag_name && $tag_id ) {
								$tag_term = get_term( $tag_id, 'at_biz_dir-tags' );
								$tag_name = ( $tag_term && ! is_wp_error( $tag_term ) ) ? $tag_term->name : '';
							}
							if ( $tag_name ) : ?>
								<button type="button" class="hbl-v2-popular-search-btn" data-tag="<?php echo esc_attr( $tag_id ); ?>">
		...
		<?php if ( $show_search || $show_sort || $show_toggle || ( $show_more_filters && ! empty( $more_filter_items ) ) ) : ?>
				<div class="hbl-v2-filters-bar">
					<div class="hbl-v2-filters-left">
						...
						<?php endif; ?>
						<?php if ( $show_more_filters && ! empty( $more_filter_items ) ) : ?>
							<div class="hbl-v2-more-filters-wrapper hbl-v2-filter-dropdown">
								...
								<div class="hbl-v2-more-filters-dropdown" role="listbox" aria-hidden="true" style="display:none;">
									<div class="hbl-v2-more-filters-list">
										<?php foreach ( $more_filter_items as $mf_item ) :
											$mf_tag_id   = ! empty( $mf_item['filter_tag'] ) ? $mf_item['filter_tag'] : 0;
											$mf_tag_name = ! empty( $mf_item['filter_text'] ) ? $mf_item['filter_text'] : '';
											if ( ! $mf_tag_name && $mf_tag_id ) {
												$mf_term = get_term( $mf_tag_id, 'at_biz_dir-tags' );
												$mf_tag_name = ( $mf_term && ! is_wp_error( $mf_term ) ) ? $mf_term->name : '';
											}
											if ( $mf_tag_name ) : ?>
												<button type="button" class="hbl-v2-more-filter-item" data-tag="<?php echo esc_attr( $mf_tag_id ); ?>" role="option">
													<?php echo esc_html( $mf_tag_name ); ?>
												</button>
											<?php endif;
										endforeach; ?>
									</div>
			...
			<div class="hbl-v2-listings-grid <?php echo 'list' === $default_view ? 'list-view' : ''; ?>">
				...
				<?php else : ?>
					<div class="hbl-v2-no-results">
						...
						<p><?php esc_html_e( 'No listings found in this location.', 'hbl' ); ?></p>
					</div>
				<?php endif; ?>
			</div>
		...
		jQuery(document).ready(function($) {
			...
			function updateActiveFilters() {
				...
				if (currentFilters.tag) {
					var activeTagBtn = $widget.find('.hbl-v2-popular-search-btn.active, .hbl-v2-more-filter-item.active');
					var tagLabel = activeTagBtn.length ? activeTagBtn.first().text().trim() : '<?php esc_html_e( 'Tag', 'hbl' ); ?>';
					tags.push({ key: 'tag', label: '<?php esc_html_e( 'Tag', 'hbl' ); ?>: ' + tagLabel });
				}
			...
			function clearFilter(key) {
				if (key === 'all') {
					...
					$widget.find('.hbl-v2-more-filter-item').removeClass('active');
			...
			$widget.find('.hbl-v2-more-filter-item').on('click', function() {
				...
				if ($(this).hasClass('active')) {
					...
					$widget.find('.hbl-v2-more-filter-item').removeClass('active');
					$(this).addClass('active');
					$widget.find('.hbl-v2-more-filters-label').text($(this).text().trim());
				}
				$widget.find('.hbl-v2-more-filters-dropdown').slideUp(200);
				$widget.find('.hbl-v2-more-filters-trigger').attr('aria-expanded', 'false');
				currentFilters.paged = 1;
